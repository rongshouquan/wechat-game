...
- 第24批敌方 4 单位（星盗艇 / 星盗炮台 / 星盗船长 / 星盗头目）**设计全部通过**："说句实话能达到8、90分的高分"、"基本都可以直接当正式素材来用"。
...
- **星盗头目：删"炮口蓄力光效"**。第24批头目身份词**特意开了个口子**——写死 "visibly charging up a big shot / glowing energy core building up at the muzzle / the brightening muzzle is the signature read"，为的是让"蓄力"一眼可读；实图炮口因此带粉色光团。**这违反了上述既有纯净体通则**。Ron 拍板：资产版关掉这个特例、回归通则——删蓄力光效句、炮口画成干净炮管，蓄力光效由引擎特效后加（届时用附录D弹种/质变特效叠）。
- 星盗头目**作为测试图**保留光团无妨（本就是设计测试、非资产）；只有**资产批**必须纯净体。
...
> 资产批使用时的统一改动：①背景段换成色键净底句（pure #ff00ff flat background）；②星盗头目按上方修正案删蓄力光效句；③其余照抄。
