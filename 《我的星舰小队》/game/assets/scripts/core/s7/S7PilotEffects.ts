...
// ⚠️ 段二 R1 五档平移注记（2026-07-12 Ron 返工令·取代 07-12 早前"封存注记"）：大节点档位
//   20/40/60/80/100 → 10/20/30/40/50 整体平移·**内容零改动**——旧"L60/80/100 封存"概念对本表作废
//   （全部天赋里程碑移入可达段）。效果行 id 后缀（_l20/_l40 等）＝运行时稳定 id 沿用历史名不随平移改。
//
// 口径（细表 §13 / 驾驶员真源 §0）：
//   ① 能力（打谁/护谁·含嘲讽/守护行为）＝起手 Lv0 即生效；
//   ② 天赋＝Lv1 解锁，Lv10/20/30/40/50 大节点强化（本表按级门取对应参数）；
...
const BUILDERS: Record<string, (lv: number, star: number) => S7EffectBlock[]> = {
  // ===== 突击组（pil01-04）=====
  pil01: (lv, star) => {
    // 炎：能力=集火血量最少；天赋「过热」连击叠伤（M3 attack_landed·断击清空·L50 只降1层）。
    const b: S7EffectBlock[] = [{ kind: 'behavior', targetingTag: 'lowest_hp_enemy', source: 'pil01' }];
    if (lv >= 1) {
      const per = nodeValue(lv, [[1, 0.02], [10, 0.03], [20, 0.04]])!;
      const cap = nodeValue(lv, [[1, 3], [30, 4], [40, 5]])!;
      const rule: S7StackRuleParam = {
        ruleId: 'pil01_overheat', on: 'attack_landed', stat: 'dmgUpPct', perStack: per, maxStacks: cap,
        breakOn: 'attack_gap', breakGapSec: 2.5, breakAction: lv >= 50 ? 'decay_1' : 'clear',
      };
  ...
  pil02: (lv, star) => {
    ...
    if (lv >= 1) {
      const v = nodeValue(lv, [[1, 0.40], [20, 0.50], [30, 0.60]])!;
      b.push({ kind: 'affix', affix: 'dmgVsLowHp', value: v, source: 'pil02' });
      // 机制批③段二 L10/L40「阈值 35%/40%」：本舰残血判定阈值覆盖词条（dmgVsLowHp/5★处决共用）。
      const thresh = nodeValue(lv, [[10, 0.35], [40, 0.40]]);
      if (thresh !== undefined) b.push({ kind: 'affix', affix: 'lowHpThresholdPct', value: thresh, source: 'pil02' });
      // 机制批③段二 L50「对残血敌暴击率+15%」（条件暴击词条）。
      if (lv >= 50) b.push({ kind: 'affix', affix: 'critRateVsLowHp', value: 0.15, source: 'pil02' });
    }
    if (star >= 3 && lv >= 1) {
  ...
  pil03: (lv, star) => {
    ...
    if (lv >= 1) {
      const eff = nodeValue(lv, [[1, 'eff_pil_cdr_15'], [10, 'eff_pil_cdr_20'], [20, 'eff_pil_cdr_25'], [30, 'eff_pil_cdr_30'], [40, 'eff_pil_cdr_35'], [50, 'eff_pil_cdr_40']])!;
  ...
  pil04: (lv, star) => {
    ...
    if (lv >= 1) {
      const eff = nodeValue(lv, [[1, 'eff_pil_zhe_dmgup'], [10, 'eff_pil_zhe_dmgup_l20'], [20, 'eff_pil_zhe_dmgup_l40'], [30, 'eff_pil_zhe_dmgup_l60'], [40, 'eff_pil_zhe_dmgup_l80'], [50, 'eff_pil_zhe_dmgup_l100']])!;
  ...
  // ===== 护卫组（pil05-08）=====
  pil05: (lv, star) => {
    // 岩：能力=光盾守护（M4 guard·起手即生效·CD 2s→L10 1.8→L50 1.5）；天赋「反震」格挡+反弹（M4 reflect）。
    const guardEff = nodeValue(lv, [[0, 'eff_pil_yan_guard'], [10, 'eff_pil_yan_guard_l20'], [50, 'eff_pil_yan_guard_l100']])!;
    const b: S7EffectBlock[] = [{ kind: 'trigger', on: 'battle_start', effectRef: guardEff, source: 'pil05' }];
    if (lv >= 1) {
      const reflectEff = nodeValue(lv, [[1, 'eff_pil_yan_reflect'], [20, 'eff_pil_yan_reflect_l40'], [30, 'eff_pil_yan_reflect_l60'], [40, 'eff_pil_yan_reflect_l80']])!;
  ...
  pil06: (lv, star) => {
    ...
    const tauntEff = nodeValue(lv, [[0, 'eff_pil_li_taunt'], [10, 'eff_pil_li_taunt_l20'], [40, 'eff_pil_li_taunt_l80']])!;
    ...
    if (lv >= 1) {
      ...
      // L50"上限+2档"语义与派生顶重合·记 §13 语义注（无独立参数可加）。
      const per = nodeValue(lv, [[1, 0.03], [20, 0.04], [30, 0.05]])!;
  ...
  pil07: (lv, star) => {
    ...
    if (lv >= 1) {
      ...
      const eff = star >= 3
        ...
        : nodeValue(lv, [[1, 'eff_pil_yue_thorns'], [10, 'eff_pil_yue_thorns_l20'], [30, 'eff_pil_yue_thorns_l60'], [40, 'eff_pil_yue_thorns_l80'], [50, 'eff_pil_yue_thorns_l100']])!;
      b.push({ kind: 'trigger', on: 'battle_start', effectRef: eff, source: 'pil07' });
    }
    // L20「被多敌攻击时反弹叠加」＝天然已满足（M4 反弹对每个攻击者独立结算·语义注 §16d·同藏 L20 先例）。
    if (star >= 5) {
  ...
  pil08: (lv, star) => {
    ...
    if (lv >= 1) {
      ...
      const eff = nodeValue(lv, [[1, 'eff_pil_cang_aura'], [10, 'eff_pil_cang_aura_l20'], [30, 'eff_pil_cang_aura_l60'], [40, 'eff_pil_cang_aura_l80']])!;
      b.push({ kind: 'trigger', on: 'battle_start', effectRef: eff, source: 'pil08' });
      if (lv >= 20) b.push({ kind: 'affix', affix: 'shieldPower', value: 0.15, source: 'pil08' }); // L20 也提升护盾效果
  ...
  pil09: (lv, star) => {
    ...
    if (lv >= 1) {
      const per = nodeValue(lv, [[1, 0.03], [20, 0.04], [40, 0.05]])!;
      const capPct = nodeValue(lv, [[1, 0.30], [10, 0.40], [30, 0.50], [50, 0.60]])!;
      const rule: S7StackRuleParam = {
  ...
  pil10: (lv, star) => {
    ...
    if (lv >= 1) {
      const v = nodeValue(lv, [[1, 0.25], [10, 0.35], [30, 0.45], [40, 0.55]])!;
      b.push({ kind: 'affix', affix: 'dmgVsHighHp', value: v, source: 'pil10' });
      // 机制批③段二 L20「阈值放宽到 >40% 血」（高血判定阈值覆盖词条）。
      if (lv >= 20) b.push({ kind: 'affix', affix: 'highHpThresholdPct', value: 0.40, source: 'pil10' });
      // 机制批③段二 L50「对高血敌暴击+15%」。
      if (lv >= 50) b.push({ kind: 'affix', affix: 'critRateVsHighHp', value: 0.15, source: 'pil10' });
    }
    if (star >= 3) {
  ...
  pil11: (lv, star) => {
    // 骁：能力=集火第一排；天赋「先锋」开战窗口增伤（M1 battle_start + dmg_up·L50 附攻速）。
    const b: S7EffectBlock[] = [{ kind: 'behavior', targetingTag: 'first_column_first', source: 'pil11' }];
    if (lv >= 1) {
      const eff = nodeValue(lv, [[1, 'eff_pil_xiao_vanguard'], [10, 'eff_pil_xiao_vanguard_l20'], [20, 'eff_pil_xiao_vanguard_l40'], [30, 'eff_pil_xiao_vanguard_l60'], [40, 'eff_pil_xiao_vanguard_l80'], [50, 'eff_pil_xiao_vanguard_l100']])!;
  ...
  pil12: (lv, star) => {
    // 翎：能力=掐最高攻；天赋「夺势」击杀后攻速窗口（M1 on_kill + atk_speed_up·L50 附暴击率）。
    const b: S7EffectBlock[] = [{ kind: 'behavior', targetingTag: 'highest_attack_enemy', source: 'pil12' }];
    if (lv >= 1) {
      const eff = nodeValue(lv, [[1, 'eff_pil_ling_asu'], [10, 'eff_pil_ling_asu_l20'], [20, 'eff_pil_ling_asu_l40'], [30, 'eff_pil_ling_asu_l60'], [40, 'eff_pil_ling_asu_l80'], [50, 'eff_pil_ling_asu_l100']])!;
  ...
  pil13: (lv, star) => {
    ...
    if (lv >= 1) {
      const v = nodeValue(lv, [[1, 0.30], [10, 0.45], [30, 0.60]])!;
      b.push({ kind: 'affix', affix: 'healVsLowHp', value: v, source: 'pil13' });
      // 机制批③段二 L20「含护盾量」/L40「盾+」：对残血友军护盾量+（0.45→0.60 跟治疗档错一档跟上·数值域定）。
      const sv = nodeValue(lv, [[20, 0.45], [40, 0.60]]);
      if (sv !== undefined) b.push({ kind: 'affix', affix: 'shieldVsLowHp', value: sv, source: 'pil13' });
      // 机制批③段二 L50「治疗溢出转护盾」（溢疗转盾=医修族同消费点）。
      if (lv >= 50) b.push({ kind: 'affix', affix: 'overhealToShieldPct', value: 1.0, source: 'pil13' });
    }
    if (star >= 3) {
  ...
  pil14: (lv, star) => {
    ...
    if (lv >= 1) {
      const amp = nodeValue(lv, [[1, 0.20], [10, 0.30], [30, 0.40], [40, 0.50]])!;
      b.push({ kind: 'affix', affix: 'buffAmpPct', value: amp, source: 'pil14' });
      if (lv >= 20) b.push({ kind: 'affix', affix: 'durationPct', value: 0.25, source: 'pil14' });
      // 机制批③段二 L50「增益也小幅加暴击」：施加增益附带暴击率 +8% rider。
      if (lv >= 50) b.push({ kind: 'affix', affix: 'buffRiderCritRate', value: 0.08, source: 'pil14' });
    }
    if (star >= 3) {
  ...
  pil15: (lv, star) => {
    ...
    if (lv >= 1) {
      const per = nodeValue(lv, [[1, 0.04], [30, 0.05], [40, 0.06]])!;
      b.push({ kind: 'affix', affix: 'coverageAmpPerAlly', value: per, source: 'pil15' });
      // L10「加成上限+」＝线性无上限件·天然已满足记语义注（§16d）。
      // 机制批③段二 L20「覆盖全队额外小回血」：周期小回血（cd10·攻×4%/s·2s·近似不设全覆盖门=记近似·复测校）。
      if (lv >= 20) b.push({ kind: 'trigger', on: 'cd', cdSec: 10, effectRef: 'eff_pil_pei15_regen', source: 'pil15' });
      // 机制批③段二 L50「覆盖全队时小幅延长辅助持续」：durationPct +25%（近似恒开·复测校）。
      if (lv >= 50) b.push({ kind: 'affix', affix: 'durationPct', value: 0.25, source: 'pil15' });
    }
    if (star >= 3) {
  ...
  pil16: (lv, star) => {
    ...
    if (lv >= 1) {
      const cnt = star >= 5 ? 99 : nodeValue(lv, [[1, 1], [10, 2], [30, 3], [50, 4]])!;
      b.push({ kind: 'affix', affix: 'healDispelCount', value: cnt, source: 'pil16' });
      // 机制批③段二 L20「可驱散硬控」。
      if (lv >= 20) b.push({ kind: 'affix', affix: 'healDispelHardControl', value: 1, source: 'pil16' });
      // 机制批③段二 L40「净化回血 攻×80%」（5★=清空全部+回血 攻×300%）。
      const heal = star >= 5 ? 3.0 : (lv >= 40 ? 0.8 : 0);
  ...
  pil17: (lv, star) => {
    ...
    // 消费点=dealDamage 团队易伤扫描·anyCloak 门）。基座对 Boss 不生效、L20 才开（真源大节点）。
    const b: S7EffectBlock[] = [{ kind: 'behavior', targetingTag: 'debuffed_first', source: 'pil17' }];
    if (lv >= 1) {
      const amp = nodeValue(lv, [[1, 0.12], [10, 0.18], [30, 0.24], [40, 0.30]])!;
      b.push({ kind: 'affix', affix: 'debuffedTakenAmpTeam', value: amp, source: 'pil17' });
      if (lv >= 20) b.push({ kind: 'affix', affix: 'debuffedTakenIncludeBoss', value: 1, source: 'pil17' });
      // L50「笼罩持续/范围+」＝被动无持续/范围概念·天然已满足记语义注（§16d）。
    }
    if (star >= 3) {
  ...
  pil18: (lv, star) => {
    ...
    if (lv >= 1) {
      const eff = nodeValue(lv, [[1, 'eff_pil_cdr_15'], [10, 'eff_pil_cdr_20'], [30, 'eff_pil_cdr_25'], [40, 'eff_pil_cdr_30']])!;
    ...
    // 机制批③段二 L20「对召唤源+25%」/L50「对源穿防 50%」（条件词条族）。
    if (lv >= 20) b.push({ kind: 'affix', affix: 'dmgVsSummonSource', value: 0.25, source: 'pil18' });
    if (lv >= 50) b.push({ kind: 'affix', affix: 'armorPenVsSummonSource', value: 0.5, source: 'pil18' });
    if (star >= 3 && lv >= 1) {
  ...
  pil19: (lv, star) => {
    ...
    if (lv >= 1) {
      b.push({ kind: 'affix', affix: 'summonCapBonus', value: nodeValue(lv, [[1, 1], [20, 2]])!, source: 'pil19' });
      b.push({ kind: 'affix', affix: 'skillHaste', value: nodeValue(lv, [[1, 0.20], [30, 0.30]])!, source: 'pil19' });
      // 机制批③段二 L10/L40「召唤物攻+15/30%」/L50「血+40%」（召唤物属性继承通道）。
      const sa = nodeValue(lv, [[10, 0.15], [40, 0.30]]);
      if (sa !== undefined) b.push({ kind: 'affix', affix: 'summonAtkPct', value: sa, source: 'pil19' });
      if (lv >= 50) b.push({ kind: 'affix', affix: 'summonHpPct', value: 0.40, source: 'pil19' });
    }
    if (star >= 3) {
  ...
  pil20: (lv, star) => {
    // 藏：能力=啃最高防；天赋「破甲」无视防御%（armorPen·L20 对Boss=词条天然全体✓）。
    const b: S7EffectBlock[] = [{ kind: 'behavior', targetingTag: 'highest_armor_enemy', source: 'pil20' }];
    if (lv >= 1) {
      const v = nodeValue(lv, [[1, 0.20], [10, 0.30], [30, 0.35], [40, 0.40]])!;
      b.push({ kind: 'affix', affix: 'armorPen', value: v, source: 'pil20' });
      if (lv >= 50) b.push({ kind: 'modifier', stat: 'attack', op: 'pct', value: 0.08, source: 'pil20' }); // L50 破甲附增伤 8%
    }
    if (star >= 3) {
