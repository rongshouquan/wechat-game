...
// 形状与打捞加速的 {dayKey,count} 同构（跨天自动重置）；dayKey = floor(now/86400000)，与 salvageDayKey 同口径。
...
/** 当日 key（与 salvageDayKey 同口径；非有限/负值归 0）。 */
export function adDayKey(now: number): number {
  return Number.isFinite(now) && now > 0 ? Math.floor(now / DAY_MS) : 0;
}
...
export function adDailyUsed(state: S7AdDailyState, point: string, now: number): number {
  ...
  if (!entry || entry.dayKey !== adDayKey(now)) return 0;
  return entry.count;
}
