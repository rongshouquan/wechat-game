...
//
// 文案池（交互巡检批接入·复验修复批定稿 72 条）：
//   - 12 条种子（块5 执行会话写·无署名的星港日常，速写体）＋ 60 条正式池（源=《星港趣事文案池-v1》·
//     Codex 产 60 条→总控 2026-07-03 调性初校→巡检批禁忌词扫描剔 006→复验修复批经 Ron 拍板改词回池：
//     006「留到最后」→「留到压轴」，语义不变、过扫描）。
...
/** 定稿文案池（72 条 = 12 种子 + 60 正式；顺序：种子 → 驾驶员 001-040(006=改词版) → 居民 041-060）。 */
const ANECDOTE_LINES: S7AnecdoteLine[] = [
  ...
  // ---- 驾驶员 40 条（v1 池 001-040·006 经 Ron 拍板改词回池："留到最后"→"留到压轴"）----
  ...
  { avatar: '⚡', speaker: '燎', text: '烬说好菜要留到压轴。行，你留，我先吃。', reward: { starOre: 4 } },
