...
describe('⑦M2 周期结算（燃烧/持续回血·快照量·手推期望值）', () => {
  ...
  it('燃烧按目标最大血：1%/s（靶 2000 血→20/s）；护盾先扛（1:1 啃盾·血不动）', async () => {
    ...
    const b2 = mechRig({
      players: [{ rowId: 'bu_t7_burner', overrides: { normalEffectRef: 'none' } }],
      // 批③段三重锚：敌方大招首放转满 CD（cd999=永不放）→ 开局自盾改单位行 battle_start 触发通道（⑦M1 敌事件通道·t0 起盾前提复原）。
      enemies: [{ rowId: 'bu_t7_shielded', slot: 'r1c0', overrides: { ultimateEffectRef: 'none', ultimateCdSec: 0, extraTriggerBlocks: [{ kind: 'trigger', on: 'battle_start', effectRef: 'eff_state_shield' }] } }],
    ...
    // 旧 19990…=盾 20000（C12 旧地板 0.2×100000）；C12 修订后全局盾行走 shieldMaxHpPct 0.25 → 盾 25000（燃烧 10/s 递减不变）。
    expect(ticks.map((e) => e.shieldAfter)).toEqual([24990, 24980, 24970, 24960]);
...
describe('⑦M3 叠层规则（过热/专注/坚甲/愈坚/贪吃星·手推期望值）', () => {
  ...
  it('铁壁「坚甲」：受"重击"(技能伤害)叠 10% 减伤/层·普攻命中不叠', async () => {
    ...
    // 批③段三重锚（敌大招首放转满 CD·机制未变）：技能弹 cd2 首放 t2 → t2 80(0层·先结再叠) t4 72(1层)——6s 窗内两发；
    // 普攻弹只吃减伤不叠层：t0/t1 80(0层) t2/t3 72(技能同拍先结=1层) t4/t5 64(2层)。旧值 [80,72,64]/[72,72,64,64,56,56]=首放 t0 时代。
    const skillHits = r.log.filter((e) => e.type === 'damage' && e.effectRef === 'eff_t7_nuke2').map((e) => e.amount);
    expect(skillHits).toEqual([80, 72]);
    const normalHits = r.log.filter((e) => e.type === 'damage' && e.actorId === 'enemy_0001' && e.effectRef === 'eff_basic_attack').map((e) => e.amount);
    expect(normalHits).toEqual([80, 80, 72, 72, 64, 64]);
  });
...
describe('⑨敌载体三缺口（组合验证·能力已由 M1-M8 完备·接线归⑥回归批·不碰敌配JSON）', () => {
  it('削弱我方（§20.6 #10 缺口）：敌 apply_state 对我方施 atk_down −50% → 我方伤 80→40', async () => {
    ...
    // 批③段三重锚：敌大招首放转满 CD → t3 才施 atk_down。前 3s 全额 80、其后 40（削弱语义未变）。
    const hits = dmgBy(r.log, 'player_p1c2');
    expect(hits.filter((e) => e.timeSec < 3).length).toBeGreaterThan(0);
    expect(hits.filter((e) => e.timeSec < 3).every((e) => e.amount === 80)).toBe(true);
    expect(hits.filter((e) => e.timeSec >= 3.2).length).toBeGreaterThan(0);
    expect(hits.filter((e) => e.timeSec >= 3.2).every((e) => e.amount === 40)).toBe(true);
  });
  ...
  it('敌方 AoE 载体：敌用 block_area+splashPct 命中我方多单位（首满额·邻半额）', async () => {
    const b = mechRig({
      ...
      // 批③段三重锚：敌大招首放转满 CD（cd99=永不放）→ cd5：6s 窗内恰好一发（断言口径不变）。
      enemies: [{ rowId: 'bu_t9m9_aoe', slot: 'r1c0', overrides: { attack: 100, ultimateEffectRef: 'eff_t9m9_ea', ultimateCdSec: 5, attackRangeCells: 99 } }],
