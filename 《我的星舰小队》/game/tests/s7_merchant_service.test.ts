// 商人小站引擎单测（步5 收尾批重定基·百货化=Ron 亲排 10 级表 + 初值表 v0.7 §6-5）——
// 重定基（旧→新→为什么对）：①轮换 5-8 槽 → 稀有格 1/2/3（Lv1/4/7）；②券价 120 限1 → 70 不限购（总修订案五）；
//   ③星空宝石轮换项退役（稀有格 6 品类=v0.7 rare.pool）；④回收 普通信标 40→25（+3/档）·星矿 4:1→8:1（步3 A1）；
//   ⑤新增 Lv10 全场九折上货价。——
...
import {
  ...
  refreshMerchantShop, recycleBeacon, recycleStarOre, beaconCommonRecyclePrice, merchantDayKey,
  grantMerchantFreeRefresh,
} from '../assets/scripts/core/s7/S7MerchantService';
import { merchantRareSlots } from '../assets/scripts/core/s7/S7BuildingEffects';
...
function ctrlCfg(): S7MerchantConfig {
  return cfg({
    alwaysOffers: [
      ...
      { item: RES('adTicket', 1), price: 70, purchaseLimit: 1, keepBoughtOnRefresh: true }, // 受控限 1=验 keepBought 机制（真值不限购·见上货测试）
    ],
    rarePool: [{ item: RES('beaconCommon', 1), price: 200, purchaseLimit: 3, rareWeight: 100 }],
    refresh: { freePerCycle: 1 },
  });
}

describe('E-step1 · 上货 / 轮换格', () => {
  it('稀有格随等级 1/2/3（Lv1/4/7=Ron 亲排 10 级表·旧 5-8 槽作废）', () => {
  ...
  it('generateMerchantStock：常驻区(5 件) + 稀有格=merchantRareSlots(lv)；lv1 无流通核(Lv5 门槛)·券价 70 不限购', () => {
    const s = createDefaultS7Merchant();
    generateMerchantStock(s, DEFAULT_S7_MERCHANT_CONFIG, 1, rng());
    // 补给券常驻·恒 o0（教程 step43 高亮依赖第一行）·日限 50（v0.7 铁律阀门）。
    const ticket = s.offers.find((o) => o.item.kind === 'resource' && o.item.resourceId === 'supplyTicket');
    expect(ticket?.offerId).toBe('o0');
    expect(ticket?.purchaseLimit).toBe(50);
    // 广告券常驻：70 星贝·不限购（总修订案五·旧 120 限1 作废）。
    const adTicket = s.offers.find((o) => o.item.kind === 'resource' && o.item.resourceId === 'adTicket');
    expect(adTicket).toBeTruthy();
    expect(S7_AD_TICKET_PRICE).toBe(70);
    expect(adTicket?.price).toBe(70);
    expect(adTicket?.purchaseLimit).toBe(S7_AD_TICKET_DAILY_BUY_LIMIT);
    expect(adTicket?.purchaseLimit).toBeNull();
    // 格数 = 常驻(5) + 稀有格(1)=6。
    expect(s.offers).toHaveLength(DEFAULT_S7_MERCHANT_CONFIG.alwaysOffers.length + merchantRareSlots(1));
    // lv1 不出流通核（Lv5 权重档门槛）。
    expect(s.offers.some((o) => o.item.kind === 'core')).toBe(false);
    // 无重复品类。
  ...
  it('高等级：稀有格 3 格·Lv5+ 可出流通核·Lv10 全场九折上货价', () => {
    let sawCore = false;
    for (let i = 0; i < 40 && !sawCore; i += 1) {
      const s = createDefaultS7Merchant();
      generateMerchantStock(s, DEFAULT_S7_MERCHANT_CONFIG, 7, new S7AutoBattleRng(`hi${i}`));
      expect(s.offers).toHaveLength(DEFAULT_S7_MERCHANT_CONFIG.alwaysOffers.length + merchantRareSlots(7)); // 5 常驻 + 3 稀有格
      sawCore = s.offers.some((o) => o.item.kind === 'core');
    }
    expect(sawCore).toBe(true); // Lv5+ 流通核入池（w0.02 低频·40 种子内应现身）
    // Lv10 全场九折：补给券 80→72。
    const s10 = createDefaultS7Merchant();
    generateMerchantStock(s10, DEFAULT_S7_MERCHANT_CONFIG, 10, rng());
    const t10 = s10.offers.find((o) => o.item.kind === 'resource' && o.item.resourceId === 'supplyTicket');
    expect(t10?.price).toBe(72); // round(80×0.9)
  });
});
...
describe('E-step1 · 刷新（去付费·全新一茬店：货+购买次数一起刷·剩余次数制）', () => {
  ...
  it('keepBoughtOnRefresh（块5 广告券）：手动刷新不清已购——每日限购1不被"免费/广告刷新重开一茬店"绕过；跨天才重置', () => {
    ...
    expect(wallet.starCargo).toBe(999999 - 70 * 2); // 受控配置券价 70
  });
...
describe('E-step1 · 回收（卖→星贝·折损）', () => {
  it('回收信标→星贝：普通 25 基价（v0.7 终值·旧 40 作废）+ 回收价档 Lv3/6/9 各 +3', () => {
    const wallet: Record<string, number> = { beaconCommon: 3, beaconRare: 1, starCargo: 0 };
    expect(recycleBeacon(wallet, DEFAULT_S7_MERCHANT_CONFIG, 'common', 2)).toMatchObject({ ok: true, starCargoGained: 50 }); // 2×25
    expect(wallet.beaconCommon).toBe(1);
    expect(wallet.starCargo).toBe(50);
    // 回收价档：Lv3=28/Lv6=31/Lv9=34（细案⑤ Lv3/6/9 回收①②③）。
    expect(beaconCommonRecyclePrice(DEFAULT_S7_MERCHANT_CONFIG, 3)).toBe(28);
    expect(beaconCommonRecyclePrice(DEFAULT_S7_MERCHANT_CONFIG, 6)).toBe(31);
    expect(beaconCommonRecyclePrice(DEFAULT_S7_MERCHANT_CONFIG, 9)).toBe(34);
    expect(recycleBeacon({ beaconCommon: 1, starCargo: 0 }, DEFAULT_S7_MERCHANT_CONFIG, 'common', 1, 9)).toMatchObject({ ok: true, starCargoGained: 34 });
  ...
  it('回收溢出星矿→星贝（每 8 星矿=1 星贝·步3 A1 终值 4:1→8:1 压死水·零头留着）', () => {
    const wallet: Record<string, number> = { starOre: 20, starCargo: 0 };
    expect(recycleStarOre(wallet, DEFAULT_S7_MERCHANT_CONFIG, 20)).toMatchObject({ ok: true, starCargoGained: 2 }); // floor(20/8)=2
    expect(wallet.starCargo).toBe(2);
    expect(wallet.starOre).toBe(4); // 扣 16·零头留着
    expect(recycleStarOre(wallet, DEFAULT_S7_MERCHANT_CONFIG, 999)).toMatchObject({ ok: false, reason: 'insufficient' });
    expect(recycleStarOre({ starOre: 7, starCargo: 0 }, DEFAULT_S7_MERCHANT_CONFIG, 7)).toMatchObject({ ok: false, reason: 'bad_amount' }); // 不足 1 星贝
  });
});
