...
describe('校验器武装（新字段带了就严校·缺席=全量样例照旧全绿）', () => {
  ...
  it('R3 星域主题规则设计律：天气型常驻合法/事件型 >50% 被抓/孤字段被抓', () => {
    const b = cloneBundle(loadBundle());
    const regions = b.star_region_config as Row[];
    regions[0].ruleKind = 'weather';
    regions[0].ruleTag = 'storm_amp'; // 天气型常驻=合法
    regions[1].ruleKind = 'event';
    regions[1].ruleTag = 'mist_ambush';
    regions[1].ruleRate = 0.6; // 事件型 >50%=违设计律
    regions[2].ruleRate = 0.3; // 孤字段（无 ruleKind）=非法
    const msgs = validateS7ConfigBundle(b as never).map((e) => e.message).join('\n');
    expect(msgs).toContain('事件型规则出现率必须 ∈(0,0.5]');
    expect(msgs).toContain('ruleTag/ruleRate 只能与 ruleKind 一起出现');
    expect(msgs).not.toContain('天气型规则=常驻'); // 天气型合法配置不误伤
    // 合法事件型（40%）零报错。
    const ok = cloneBundle(loadBundle());
    const r = (ok.star_region_config as Row[])[2];
    r.ruleKind = 'event'; r.ruleTag = 'mist_ambush'; r.ruleRate = 0.4;
    expect(validateS7ConfigBundle(ok as never)).toEqual([]);
  });
});
