...
describe('S7Squad · 拥有与编队操作', () => {
  it('默认空（含 shipLoadouts）；grant 幂等', () => {
    ...
    expect(s).toEqual({ ownedShips: [], ownedPilots: [], ownedCores: {}, formation: [], shipLoadouts: {}, lineups: {} });
...
describe('S7Squad · 分玩法阵容记忆（③b·Ron 2026-07-05）', () => {
  it('首进某玩法从当前 formation 播种（迁移），squad.formation 不变', () => {
    const s = createDefaultS7Squad();
    s.formation = [{ slotRef: 'p0c2', shipId: 'shp01' }, { slotRef: 'p1c2', shipId: 'shp02' }];
    const loaded = loadGameplayLineup(s, 'mainline');
    expect(s.lineups.mainline).toEqual(s.formation); // 播种
    expect(loaded).toEqual(s.formation);
    expect(s.lineups.mainline).not.toBe(s.formation); // 深拷贝（非同引用·防串改）
  });

  it('四把钥匙各记各的·跨玩法不串', () => {
    const s = createDefaultS7Squad();
    s.formation = [{ slotRef: 'p0c2', shipId: 'shp01' }];
    // 主线记 shp01；进回廊(从主线播种)后改成 shp02；再回主线仍是 shp01。
    loadGameplayLineup(s, 'mainline');
    saveGameplayLineup(s, 'mainline');
    loadGameplayLineup(s, 'corridor'); // 首进回廊·从当前(主线 shp01)播种
    expect(s.formation).toEqual([{ slotRef: 'p0c2', shipId: 'shp01' }]);
    s.formation = [{ slotRef: 'p2c0', shipId: 'shp02' }]; // 回廊改阵
    saveGameplayLineup(s, 'corridor');
    // 护航/演习各自独立播种。
    loadGameplayLineup(s, 'escort'); saveGameplayLineup(s, 'escort');
    // 回主线：仍是 shp01（没被回廊/护航串改）。
    loadGameplayLineup(s, 'mainline');
    expect(s.formation).toEqual([{ slotRef: 'p0c2', shipId: 'shp01' }]);
    // 回回廊：仍是回廊改后的 shp02。
    loadGameplayLineup(s, 'corridor');
    expect(s.formation).toEqual([{ slotRef: 'p2c0', shipId: 'shp02' }]);
  });

  it('规范化 round-trip lineups（脏值丢弃·≤5·去重）', () => {
    const s = normalizeS7Squad({
      formation: [{ slotRef: 'p0c2', shipId: 'shp01' }],
      lineups: {
        mainline: [{ slotRef: 'p0c2', shipId: 'shp01' }, { slotRef: 'bad', shipId: 'x' }], // bad 丢弃
        corridor: [{ slotRef: 'p1c1', shipId: 'shp03' }],
        junk: [{ slotRef: 'p0c0', shipId: 'shp09' }], // 非法钥匙丢弃
      },
    });
    expect(s.lineups.mainline).toEqual([{ slotRef: 'p0c2', shipId: 'shp01' }]);
    expect(s.lineups.corridor).toEqual([{ slotRef: 'p1c1', shipId: 'shp03' }]);
    expect((s.lineups as Record<string, unknown>).junk).toBeUndefined();
  });

  it('老档无 lineups 字段 → 规范化补空（加性）', () => {
    const s = normalizeS7Squad({ formation: [{ slotRef: 'p0c2', shipId: 'shp01' }] });
