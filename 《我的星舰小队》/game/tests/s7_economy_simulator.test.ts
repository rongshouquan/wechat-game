...
describe('S7 经济尺 · 战力刻度 v1（定价重锚批实测重标·镜像 S7PowerRating）', () => {
  ...
  it('等级因子=LF 表（growth_band 每轴倍数×节点门 L20/L40）：B阶15级=108×1.8213', () => {
    const base = shipBasePower(1, 15);
    expect(base).toBeCloseTo(108 * 1.8213, 1);
    // L20 节点门（×1.100 已折进 LF 表）：LF(20)=1.9833×1.100=2.1816
    expect(shipBasePower(0, 20)).toBeCloseTo(100 * 2.1816, 1);
...
describe('S7 经济尺 · 显示推荐值==压力表（对锚批拍板⑦守卫）', () => {
  it('pressure_param 逐节点行与校准压力表逐点一致（段2a 过渡：精确钉转 ≤10% 漂移带·2b 重落恢复）', () => {
    // 为什么对：拍板⑦"显示推荐=真实需求"——逐节点行由 apply-pressure-display.mjs 落数；本守卫
    // 防"压力表重校后显示行忘记重落"。
    // ⚠️ 段2a 过渡豁免（同 07-11 旧靶豁免制·到期条件=段二 2b 400 关压力表+显示行整体重落时恢复
    // 精确钉）：A3 等级上限 100→50 使校准器自愈重校（普通档轨迹变→γ 重解），150 关旧显示行=
    // 旧世界落数，全量重落被任务单明排在 2b（400 关一次重铺·不在半途世界重落两遍）。
    // 过渡期底线仍武装：逐点漂移必须 ≤10%（重校幅度级）——出现倍级漂移=真 bug 照红；
    // 漂移条目打印留档（同豁免制打印纪律）。
    ...
    const check = (label: string, disk: number | undefined, live: number) => {
      ...
      expect(Math.abs(live - d) / d, `${label} 超过渡漂移带 10%`).toBeLessThanOrEqual(0.10);
    };
...
describe('S7 经济尺 · 压力值表结构（双锚 v2）', () => {
  ...
  it('6 真Boss墙位于 60/84/102/120/138/150；4 面真墙跳升（段2a 过渡：1.10 地板转 1.05+打印）、84/138 余势近平', () => {
    // ⚠️ 段2a 过渡豁免（同旧靶豁免制·到期=2b 400 关新墙靶落地恢复）：旧 1.10 跳升地板=
    // 150 关旧世界墙形状靶（07-11 收口已宣布该靶系世界作废）；A3 L50 重校后实测 n120 跳升
    // 1.095 贴地板下沿（校准器自愈幅度级·非结构劣化）。过渡底线仍武装：真墙必须仍是"峰"
    // （跳升 >1.05 且 ≥ 余势墙水平），跌破=压力表结构真坏照红；实测值打印留档。
    expect(TRUTHS.bossNodes).toEqual([60, 84, 102, 120, 138, 150]);
    const jumps: string[] = [];
    for (const w of [60, 102, 120, 150]) {
      const j = pressure[w] / pressure[w - 1];
      jumps.push(`n${w}=${j.toFixed(3)}`);
      expect(j, `n${w} 墙跳升塌方（过渡底线 1.05）`).toBeGreaterThan(1.05);
    }
    // eslint-disable-next-line no-console
    console.log(`[旧靶豁免·段2a 过渡] 真墙跳升实测（旧地板 1.10·2b 重钉）：${jumps.join(' ')}`);
...
// ===== 定价重锚批收口 · 旧靶豁免制（总控收口指令 07-11 晚·作废记档模式，不是装绿）=====
// 旧→新→为什么对→到期条件：
//   旧靶 = Ron 07-10 十六格墙矩阵连环拍（"47 天/150 关"世界的靶系：矩阵点靶±1/余势零墙/
//   双轴单调/毕业日带·含抗漂移变体承诺的同款靶）；
//   作废依据 = Ron 07-11 晚新总纲连环拍（普通档 2 个月毕业/主线 400 关/毕业态转正=D47 实测态/
//   等级上限 100→50 舰员两线同改/大节点每 10 级）——旧靶所在世界不复存在，段二在新世界重钉全套靶；
//   豁免范围 = 仅"靶系"断言（下方家族名单）——守恒/硬顶/新手零墙/首周/档位顺序/B1 身份/
//   未毕业检测等仍然有效的法条保持武装，破了照红；豁免条目照常打印全文（数据留档）；
//   到期条件 = 段二新靶落地时删除本豁免、恢复严格断言。
// '变体容忍' 也入豁免家族的依据（收口时补·总控验收请重点核这一条）：它=抗漂移变体承诺的
// "轻度瞬时 ≤9"带（HARD_WALL_CAP_DRIFT_LIGHT·对锚批围绕旧校准建的变体机构·salvage×0.8 下
// 轻度 n150 事件墙读数 10>9）——Ron 绝对线"任何单墙 ≤7 基线硬顶"在校准基线测试里保持武装
// 且当前绿（轻度 n150 基线=5 天），豁免的只是旧世界变体带读数；段二新靶落地时随豁免制一并恢复。
const OBSOLETE_TARGET_FAMILIES = ['墙矩阵', '余势关', '墙轴单调破形', '档轴单调破形', '偏离靶', '变体容忍'];
function exemptObsoleteTargets(errors: string[], tag: string): string[] {
  const exempted = errors.filter((e) => OBSOLETE_TARGET_FAMILIES.some((f) => e.includes(f)));
  // eslint-disable-next-line no-console
  if (exempted.length) console.log(`[旧靶豁免·${tag}] ${exempted.length} 条（段二新世界重钉·收口指令 07-11）：\n  ${exempted.join('\n  ')}`);
  return errors.filter((e) => !OBSOLETE_TARGET_FAMILIES.some((f) => e.includes(f)));
}

describe('S7 经济尺 · 校准基线（旧靶豁免制·非靶系法条武装）', () => {
  it('四档验收：守恒/硬顶/新手零墙/首周/档位顺序/B1 身份全过；旧十六格靶系=豁免记档（段二重钉）', () => {
    // 为什么对：checkCalibration 的判定分两族——"靶系"（矩阵/单调/毕业日带=07-10 旧世界拍板·
    // 随 07-11 新总纲整体作废）与"法条系"（守恒/硬顶/新手零墙/首周/档位顺序/B1=世界无关的硬规矩）。
    // 豁免前者、武装后者=按真实状态重定基（收口指令第 1 条）。
    expect(exemptObsoleteTargets(checkCalibration(std), '校准基线')).toEqual([]);
  });
...
describe('S7 经济尺 · 黑市（S13.6 入模 · 任务单③验收三条）', () => {
  ...
  it('黑市党毕业=D26 钉住（Ron 带 [22,27] 内·回访钩子按设计触发·段2a L50 重校记档）', () => {
    ...
    // 段2a 回访记录（钉死钩子按设计转红·旧→新→为什么对）：D27→D26——A3 等级上限 100→50 使
    // 中后期等级钱坑变浅+校准器自愈重校，黑市党提前 1 天毕业；仍在 Ron 拍A 带 [22,27] 内=
    // 拍板未破，随段2a 交付回访报总控/Ron 知悉。继续钉死新值：任何再变动照旧转红强制回访。
    expect(BM_TARGET.min).toBe(22);
    expect(BM_TARGET.max).toBe(27); // Ron 2026-07-08 拍A
    expect(bm.graduateDay).toBe(26); // 回访后钉新值（钩子保留·带内）
  });
...
describe('S7 经济尺 · 抗漂移回归护栏（任务单③硬规格 #2 · 旧靶豁免制同款）', () => {
  // 为什么对：护栏跑的是"扰动参数 → 全流程重校准 → 变体承诺验收"。判定项里
  // 矩阵 DRIFT 带/毕业日带=旧世界靶系（07-11 新总纲作废·豁免记档·段二重钉——新纸面
  // "破墙锁升阶/升星事件日"的相位在 ±20% 扰动下跳 ±3 天=旧带装不下，见细表 §16f）；
  // 守恒/硬顶（含轻度变体容忍 ≤9）/新手零墙/未毕业检测=法条系保持武装。
  for (const v of DRIFT_VARIANTS) {
    it(`${v.source}×${v.mult}：重校后守恒/硬顶/新手零墙法条全过；旧带靶系=豁免记档`, () => {
      expect(exemptObsoleteTargets(runDriftVariant(v.source, v.mult).errors, `漂移${v.source}×${v.mult}`)).toEqual([]);
    });
  }

  it('指定反例 treasure×2（中后期变富·v0.1 病灶场景）：法条全过；肝 n060 旧带断言=豁免记档', () => {
    // 为什么对：v0.1 的病=行动宝藏入模让中后期变富 → 单γ全曲线下调 → 肝档 n060 墙
    // 4→5 破口径；双锚改造的对抗验证保留（法条系武装）。旧附加断言 liverWalls[60]≤2=
    // 十六格靶系（07-11 新总纲作废·数据照打印·段二新靶落地时恢复严格断言）。
    const r = runDriftVariant('treasure', 2);
    expect(exemptObsoleteTargets(r.errors, '漂移treasure×2')).toEqual([]);
    // eslint-disable-next-line no-console
    console.log(`[旧靶豁免·treasure×2] 肝 n060 墙=${r.liverWalls[60]} 天（旧带 ≤2·段二重钉）`);
  });
});
