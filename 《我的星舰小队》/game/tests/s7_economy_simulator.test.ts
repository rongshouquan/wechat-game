...
describe('S7 经济尺 · 建筑细案入尺批（八栋九件·手推期望值 + 接线直测）', () => {
  ...
  it('⑥ 免费抽接线实证：关掉免费抽 → 同压力表下专属碎片流严格变少', () => {
    ...
    // 定价重锚 v1 重定基：相对阈 ×0.995 → 绝对降幅 ≥20 碎片——回廊 req 重锚恢复层轨迹后
    // 总碎片流变大（~8.7k），免费抽同样的绝对贡献（~36 碎片）相对占比 0.41% 跌破旧相对阈=
    // 假红；绝对降幅口径直接对准"每抽 2 碎片×日 1-2 抽"语义、对总流规模不敏感。
    ...
    expect(shardFlow(off)).toBeLessThan(shardFlow(std['普通'].expected) - 20);
  });
