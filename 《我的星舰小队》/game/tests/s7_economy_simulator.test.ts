...
describe('S7 经济尺 · 校准基线全绿（带判定·不钉具体天数）', () => {
  ...
  it('黑市党验收=只剩已知带冲突一条（硬顶/零墙/账本全过·⑧交回项钉住）', () => {
    // 为什么对：checkBlackMarket 的其余全部判定（硬顶/新手零墙/守恒/计数账本/日上限）必须
    // 全绿；唯一允许在场的是已知带冲突字符串（上测钉住 D26 待 Ron 拍）——任何第二条错误
    // 或错误文案变化都转红。
    expect(checkBlackMarket(std[BM_TARGET.tier].expected)).toEqual(['黑市党毕业 D27 超出靶带 D22-25']);
  });
...
describe('S7 经济尺 · 黑市（S13.6 入模 · 任务单③验收三条）', () => {
  ...
  it('黑市党毕业=D26（⑧已知结构冲突·钉住待 Ron 拍——带常量 [22,25] 未动）', () => {
    // 为什么这样测（任务单⑧交回项·§18 全数据）：D22-25 带=硬规格"不许动"，⑧新经济下（含 #10 幽灵收入修正后）
    // 黑市党模型最优=D26——五个域内杠杆全数实测推不动（计数+50%/解锁提前 10 关/完全不买
    // 宝箱/宝箱+43%/券+1 → 全部 D26）：主力 ≈D20 SS 饱和后多余碎片溢板凳（3% 封顶）边际
    // 归零，末墙物理=军饷成长速度（与肝档同物理·被普通档锚定的压力表锁死），watcher 通用
    // 增益全被重校吃掉。带 vs 结构的取舍=Ron 拍（候选：带改 [22,26] / 改"宝箱必买"策略钉 /
    // 黑市党专属供给）。本测钉住 D26：变好/变坏都转红=拍板后强制回访；带常量守门见下测。
    expect(BM_TARGET.min).toBe(22);
    expect(BM_TARGET.max).toBe(25); // 带一字未动（"不许动"合规）
    expect(bm.graduateDay).toBe(27); // 已知冲突钉住（非验收放行）
  });
