...
describe('⑩A1-接线门扫描（级门取对档·星门开质变·§13 参数逐点）', () => {
  ...
  it('燎连斩级门（R1 五档平移）：Lv1→1.5s / Lv10→2s / Lv50→4s；3★ 加连杀攻速叠层', () => {
    // R1 重定基（旧→新→为什么对）：大节点档位 20/40/60/80/100→10/20/30/40/50 整体平移·内容零改动
    // （Ron 2026-07-12 返工令）——级门断言随平移半移；效果 id=历史名不变。
    expect(trigRefs(pilotBlocks('pil03', 1))).toContain('eff_pil_cdr_15');
    expect(trigRefs(pilotBlocks('pil03', 10))).toContain('eff_pil_cdr_20');
    expect(trigRefs(pilotBlocks('pil03', 50))).toContain('eff_pil_cdr_40');
    expect(trigRefs(pilotBlocks('pil03', 9))).not.toContain('eff_pil_cdr_20'); // 门前不取（平移探针）
  ...
  it('影斩首级门（R1 五档平移）：Lv1=+40% → Lv20=+50% → Lv30=+60%（dmgVsLowHp）', () => {
    // R1 重定基：旧 Lv40/Lv60 档 → Lv20/Lv30（平移·值不变）。
    expect(affixOf(pilotBlocks('pil02', 1), 'dmgVsLowHp')).toBe(0.40);
    expect(affixOf(pilotBlocks('pil02', 20), 'dmgVsLowHp')).toBe(0.50);
    expect(affixOf(pilotBlocks('pil02', 30), 'dmgVsLowHp')).toBe(0.60);
    expect(affixOf(pilotBlocks('pil02', 19), 'dmgVsLowHp')).toBe(0.40); // 门前不升档（平移探针）
  });
