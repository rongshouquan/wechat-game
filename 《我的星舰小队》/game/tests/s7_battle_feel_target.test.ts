...
describe('⑥实证C · 星核强度表（新分档口径·毕业核高一档+常规核逆袭）', () => {
  ...
  const CORES: Record<string, { blocks: S7EffectBlock[]; carrier?: string }> = {
    ...
    超新星: { // 毕业核：攒 14s → 全屏 ×5.0（⑥实证校准 20s/×4.0→14s/×5.0·细表§15 同步）
      // 段5 重定基（旧→新→为什么对）：旧=恒速快照（A 案前与 core15 一致）；新+killCdReduceSec 1.5/
      // killCdFloorSec 6=现役 core15 同款（S7CoreEffects·Ron 07-13 A 案）——判线守卫必须测发货行为，
      // 恒速快照在 A 案后=守卫盯旧行为的假绿。
      blocks: [{ kind: 'trigger', on: 'cd', cdSec: 14, initialCdSec: 14, killCdReduceSec: 1.5, killCdFloorSec: 6, effectRef: 'eff_s7_supernova', source: 'core_nova' }],
    },
