...
// 段2 进展（恢复范围记账·两条件之一已成）：WALL_BOOST 九墙已手调落值（n104-n450 终值+3 处偏差/
//   停手记档=战斗篇战斗批账）——本组的"九墙号+boost 现值"前置就绪；剩余硬依赖=段 6 初见口径
//   全线实测产出的**新冻结态快照**（D 日 mains+逐日爬坡带=快照本体·未实测无从钉值）。
// 到期条件（余一半）=段 6 初见实测后按 450 关九墙+新快照重建本组（s7-wall-climb.mjs 复算口径
//   不变·恢复时删本注记）。带宽中档基线组（上方）=引擎行为守卫，不受影响照跑。
