...
describe('块4a 解析器 pluginBlocks：品质缩放 + 传奇额外效果', () => {
  ...
  it('技能(CD)槽=缩短攻击间隔(负pct)、战术槽=加护甲(正pct)', () => {
    const skill = pluginBlocks('plg07', 'skill', 'fine')[0] as S7ModifierBlock;
    expect(skill.stat).toBe('attackIntervalSec');
    expect(skill.value).toBeLessThan(0); // 负 = 缩 CD / 提速
...
describe('块4a 引擎：装武器插件 → 每发伤害变高（带对照防假过）', () => {
  ...
  it('对照：技能(CD)槽插件 → 普攻间隔变短、单位时间内攻击次数更多（防假过）', async () => {
    ...
    const hasted = await countAtks(pluginBlocks('plg07', 'skill', 'legendary'));
