...
describe('块4a 解析器 pluginBlocks：品质缩放 + 传奇额外效果', () => {
  ...
  it('传奇额外加一条小效果(affix)，精良/优秀没有', () => {
    expect(pluginBlocks('plg02', 'weapon', 'fine').some((x) => x.kind === 'affix')).toBe(false);
    expect(pluginBlocks('plg02', 'weapon', 'superior').some((x) => x.kind === 'affix')).toBe(false);
    const leg = pluginBlocks('plg02', 'weapon', 'legendary');
    const affix = leg.find((x) => x.kind === 'affix') as S7AffixBlock | undefined;
    expect(affix).toBeDefined();
    expect(affix!.value).toBeGreaterThan(0);
  });

  it('技能(CD)槽=缩短攻击间隔(负pct)、战术槽=加护甲(正pct)', () => {
    const energy = pluginBlocks('plg07', 'energy', 'fine')[0] as S7ModifierBlock;
    expect(energy.stat).toBe('attackIntervalSec');
    expect(energy.value).toBeLessThan(0); // 负 = 缩 CD / 提速
    const tactical = pluginBlocks('plg01', 'tactical', 'fine')[0] as S7ModifierBlock;
    expect(tactical.stat).toBe('armor');
    expect(tactical.value).toBeGreaterThan(0);
  });
...
describe('块4a 引擎：装武器插件 → 每发伤害变高（带对照防假过）', () => {
  ...
  it('对照：技能(CD)槽插件 → 普攻间隔变短、单位时间内攻击次数更多（防假过）', async () => {
    ...
    const hasted = await countAtks(pluginBlocks('plg07', 'energy', 'legendary'));
    expect(hasted).toBeGreaterThan(base); // 缩 CD → 同样时长内打更多发
  });
});
...
describe('块4a 装配器：lineup.plugins → effectBlocks + 校验', () => {
  ...
  it('3 个不同槽位插件全部装配（武器+技能+战术）', async () => {
    ...
    expect(stats).toContain('attackIntervalSec');
