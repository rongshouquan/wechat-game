...
describe('s7 battle-rt config tables (BATTLE-RT-03)', () => {
  ...
  it('lands the 5 battle tables with 2c批量production(2026-07-02)后的行数', () => {
    ...
    // 段4 首件偏差续调重定基（旧→新→为什么对）：n356 白板化续调 R1=双源结构（速鼓+攻鼓各一·
    //   分摊集火窗）→节点行 +1（bu_n356_rally_source）=857→858。
    // 段4 眼段域规则重铺重定基（旧→新→为什么对）：EYE_RULES 50 关落盘——
    //   单位行 858→906（+48=assembly 关〔25+n450〕各 +母舰节点行+sadd 产出行·apply 打印
    //   「节点行 769+add 3+sadd 77」对账）；效果行 362→387（+24 母舰召唤效果+1 风暴增幅全局行）；
    //   spawn 840→866（+25 assembly 母舰波+n450 六叠 adds 1→2 波）；graveyard 字段=节点行属性
    //   不加行；tide/surge/battlewave=encounter 字段不加行。
    expect(readTable('battle_unit_stat_param')).toHaveLength(906);
