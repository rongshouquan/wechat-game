...
describe('⑩A2-接线门扫描 · 16 核全表（9 接线+陨星弹 / 6 深坑挂牌=空）', () => {
  ...
  it('6 深坑核（core17-22）=空积木（⑨如实交回·机制批③）；未知 id 空；陨星弹原样', () => {
    for (const id of ['core17', 'core18', 'core19', 'core20', 'core21', 'core22', 'core99']) {
      expect(coreBlocks(id), `${id} 应挂牌返回空`).toHaveLength(0);
    }
