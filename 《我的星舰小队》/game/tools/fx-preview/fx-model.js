  ...
  var S7FxPlayModel = class {
    ...
    restart() {
      ...
      for (const u of this.unitList) {
        u.hpPct = 100;
        u.alive = true;
        u.present = this.presentAtStart[u.unitId] ?? u.side === "player";
        u.flashT = 0;
        u.shakeT = 0;
        u.recoilT = 0;
        u.lungeT = 0;
    ...
    introOffsetY(u) {
      ...
      const k = Math.min(1, -this.t / S7FX_INTRO_SEC);
      return k * k * S7FX_REF_H * 0.62;
    }
    ...
    exec(c) {
      ...
      switch (c.kind) {
        ...
        case "projectile": {
          ...
          this.sfxQueue.push(
            c.spec.color === "#7ED957" || c.spec.shape === "bubble" ? "battle_shoot_support" : c.spec.shape === "shell" || c.spec.shape === "ring" ? "battle_shoot_heavy" : "battle_shoot_light"
          );
