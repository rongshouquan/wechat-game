// 段二 · 最优解玩家 60 天成长剧本生成器（模板=v1.2 骨架冻结版·输出对齐现行 v1.3 带写盘闸——
// 07-15 战斗批段2：输出名 v1.2→v1.3+闸防误覆盖手工注记层+精英位 n018→n020；全量模板重铺=2c 批）。
...
export const ELITES_V11 = [
  ...
  // n018→n020（总控裁 07-15 战斗批段2 落）：18=保护期 reset_gate 非战斗节点与精英位物理不兼容（剧本层笔误）。
  { node: 7, kind: '词缀点名（纯）', diff: '微阻滞', note: 'D1 微阻滞①' }, { node: 20, kind: '奇阵·贴脸（纯）', diff: '微阻滞', note: 'D1 微阻滞②' },
...
// 段2 写盘闸（07-15·治 README 清理批记档欠账"旧名还魂/误覆盖现行 v1.3"）：磁盘剧本 md/JSON
// 含收口批手工注记层（07-14 机械同步版），本生成器模板仍=v1.2 骨架冻结版（口径横幅·全量
// 重铺=2c 批）——无闸直跑会用 v1.2 模板冲掉手工层。--force 显式越闸（2c 模板补齐后使用）。
const FORCE = process.argv.includes('--force');
function guardedWrite(file, content, handMark) {
  if (!FORCE && existsSync(file) && readFileSync(file, 'utf-8').includes(handMark)) {
    console.error(`[写盘闸] ${path.basename(file)} 含收口手工注记层（标记="${handMark}"）——模板=v1.2 骨架冻结版，拒绝覆盖（全量重铺=2c·确认重生先 git 备份再 --force）`);
    process.exitCode = 1;
    return;
  }
  writeFileSync(file, content, 'utf-8');
}
guardedWrite(path.join(HERE, '段二-最优解玩家60天剧本-数据.json'), JSON.stringify(json, null, 1), 'v13Note');
...
// 输出名对齐现行版号 v1.3（旧 v1.2 名=已删文件·防还魂）；写盘闸防冲掉 v1.3 手工注记层。
guardedWrite(path.join(HERE, '段二-最优解玩家60天剧本-v1.3.md'), md.join('\n'), 'v1.3 变更注记');
...
console.log(`生成计算完成（模板=v1.2 骨架冻结版）：60 行 ✓ 毕业 D41（≥40 铁线 ✓）卡墙 ${wallDays.total} 天（机器算）✓ 校验器 M1-M10 全过 ✓ 星核 ${coreCount} 颗（渠道账对平）✓ 毕业核 D30（R12）✓`);
console.log(`写盘目标：剧本 v1.3 md + JSON 镜像（两件带写盘闸·被拦=退出码 1）+ 普通玩家对照摘要`);
