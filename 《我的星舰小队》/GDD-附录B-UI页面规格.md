...
> **B5.1 星港悬赏板已建灰盒**（第2.5块·块2）转为"照实记录"；B5.2 深空回廊 / B5.3 每日推演仍是"目标规格"（第2.5块后续块建），工程按此建灰盒，出图排第 4 批。
...
- **⑤ 交互**：`出战` → **进主线同款备战**（默认上次阵容·敌情预览切悬赏敌阵·信息行挂**本场词缀全文**=灰盒词缀标记）→ 出战 → **场场真打**（2×/3× 倍速可用·看词缀生效找策略）→ 胜：结算窗**单键「返回悬赏板」**（藏"选关/下一关"·rule⑨）→ 返回后卡从板上消失、产出入账；负：单键返回·卡留板·不罚。护航触发**遇袭**→结算窗提示"遭遇星盗"→返回后接一场遭遇战→打完再单键返回板。🔊dispatch/🔊reward。
