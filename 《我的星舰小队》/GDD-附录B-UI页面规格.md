...
> **通用规格（所有图都遵守）**：竖屏手机比例；顶部留安全区（胶囊菜单区不画控件）；风格照 B0.1（明亮轻科技+Q萌+偏卡通·大圆角厚描边·暖橙主按钮·Q弹反馈）；详情/装配类页用**附录D 样张（极焰/磐石/炎等·2026-07-07 同步真源现名，旧"赤焰/赫炎"作废）当占位示例**填内容；全中文标注；**不画 DEV-TEMP 工具行**（正式版没有）；每图标注关键控件名。
...
| 3 | 战场·战斗一帧 | B1.3 | 我方下/敌方上 + 炮火投射特效 + 倍速键 + 特效强度反馈（不做伤害飘字·演出口径=附录D 战斗视觉总谱） |
| 4 | 单舰装配 | B1.4 | 驾驶员/星核/3插件槽 + 实时战力牌 + 品质色边（示例：极焰） |
...
| 8 | 单位管理面板 | B2.3 | 详情（阶级/等级/槽位/战力）+ 升级/升阶/装配（示例：极焰） |
...
## B0.6 · 交互通则（2026-07-04 立 · 2026-07-06 从第一原理扩至 17 条对标头部游戏 · 所有新界面从出生守规矩，交互巡检批按此验收）
...
13. **阵位方向统一换算**（2026-07-05·Ron 拍板·防再犯）：所有阵位/摆位类界面（备战九宫格 / 推演摆位 / 敌情沙盘 …）的"格子索引→屏幕位置"**必须与战斗演出朝向一致，且一律走统一换算 helper**（`S7BattleFieldOrient.s7FieldVisualCell` / `s7FieldUniformPos`，见 **B0.7**），**禁止各界面自写坐标映射**（推演页九宫格转向 bug 教训）。战斗演出 `fieldPlayerPos/fieldEnemyPos` 亦调用同一 helper=同一真源。
14. **弹窗按钮组统一语法**（2026-07-06 Ron 点名立规）：同一弹窗内按钮**等宽等高、同一水平轴、对称分布**——1 键居中、2 键左右对称等分、3 键等分一行；语义位置固定：**主行动/确认键恒在右（单键则居中）、取消/返回键恒在左**；主键暖色实心、次键冷色/描边，全游戏不反转（反例：两键弹窗一个居中一个靠右的不对称混排）。**实现唯一真源（巡检批 2026-07-06 立）**：`S7DemoController.dialogBtnRowXs`（1/2/3 键孔位）+ `addDialogBtnPair`（静态两键）+ `layoutResultKeys`（结算窗随语境重排）——今后弹窗按钮组一律走它们排布，禁止手摆坐标。**#14b 扩展到全屏页底部按钮栏（2026-07-06 复验后立·总控拍板·⚠️正式版施工规范，灰盒不搬家）**：非弹窗的全屏/半屏页，底部按钮栏同此语法——**返回/退出类恒左、主行动恒右、等宽等高对称；只有单键返回的页面=底部居中**；**全屏页不把返回放左上角或角落**（拇指热区原则+全游戏一套语法）。同走孔位函数，禁止手摆。**执行时机口径（Ron 2026-07-06）**：灰盒阶段按钮位置非定稿（正式版按附录B+美术整页重排），本条**只立规则不在灰盒搬家**——美术阶段各页落地时照此施工；灰盒只修"妨碍使用"级问题（重叠/误触/延迟），不做位置美学调整。
15. **热区达标、相邻不打架**：可点控件触摸热区 ≥88×88 逻辑像素（视觉图标可以小、热区外扩补足）；相邻可点热区**不重叠**且间距 ≥8px（反例：块5 打捞行广告键与 DEV 键曾重叠 35px）。
16. **同套件同节奏**：全游戏共用一套弹窗件——遮罩透明度、卡片圆角/描边、关闭钮位置、边距间距档（8/16/24 逻辑像素）统一；同一功能同一图标同一叫法（"返回"不混用"关闭/退出"）。
17. **可点性可预期**：可点的必须一眼看出可点（按钮质感/描边/高亮），不可点的绝不做成按钮样式；纯展示信息不带按钮底、不响应点击。
18. **「返回星港×选择关卡」三键次序**（2026-07-16 Ron 立）：凡"返回星港"与"选择关卡"两键同屏出现的界面（主线战斗结算页等）——**①返回星港恒左 ②选择关卡恒中 ③右位=该场景的主行动键**（如"下一关"）；与 #14 语法自洽（返回类恒左/主行动恒右·本条把"选择关卡"钉死居中）。**现状主线结算页为"选择关卡左/返回星港中"＝Ron 点名对调**——修正项归工程灰盒批（Ron 亲点＝不适用 #14b"灰盒不搬家"豁免）。

> **边界口径（答 2026-07-06 Ron 问）**：B0.6 管"交互与一致性**规则**"（按钮语法/热区/套件统一都算），像素级美感与配色归美术阶段和各页规格——所以"两键一个居中一个靠右"正属 B0.6 该抓的问题，已立为 #14。
> **画布标定（巡检批 2026-07-06 实测）**：灰盒运行时可视宽 = **750 设计像素**（fitWidth·高随机型变，布局须锚 `usableTopY/usableBottomY`）——由块5"打捞行 35px 重叠"反推标定（0.26W+115 − (0.44W−55) = 35 → W=750）。今后算热区重叠/间距一律按 W=750 校，别拿 720 或 1280 猜。
> **✅ 交互巡检批（2026-07-06）已按 17 条扫全部存量界面**：36 界面 × 17 条对照矩阵 + 修复清单见《进度日志-C1改造》巡检批条目；#15 的 88×88 全面达标与 #16 套件级统一（遮罩透明度/圆角归一）列美术阶段待办，其余违规已清零。
...
## B0.7 · 战场空间公约（2026-07-05·Ron 拍板·唯一朝向真源·防再犯）

> 所有阵位/摆位类界面（备战九宫格 / 推演摆位 / 敌情沙盘 / 战斗演出）**必须遵此朝向**，且走统一换算 helper（`S7BattleFieldOrient`），禁止自写（B0.6 #13）。

```
        ┌─────────────────┐   ← 屏幕上方
        │   敌方区（上半）   │
        │  最深后排 c6 ↑顶  │   敌方：列 col 大=后排(在上) / col 0=前排(在下·贴近你)
        │   敌前排 c0 ↓底   │
        ├────── 中线 ──────┤   ← 两军前排在中线相望
        │   我前排 c2 ↑顶   │   我方：列 col 大(c2)=前排(在上·贴近敌) / col 0=后排(在下)
        │  最后排 c0 ↓底    │
        │   我方区（下半）   │
        └─────────────────┘   ← 屏幕下方
              ← row 左   row 右 →         行 row = 横排(左右)、列 col = 深度(远近)
```

**四句话**：
1. **纵轴 = 深度**（远近）、横轴 = 横排（左右）；引擎格 `p{row}c{col}`/`r{row}c{col}` 里 **row=横排、col=深度**。
2. **我方在下、敌方在上**，两军**前排在屏幕中线相望**。
3. **我方前排 = 我方区最上一行**（引擎 c2·贴近敌）；**敌方前排 = 敌方区最下一行**（引擎 c0·贴近我）。
4. **统一换算**：`s7FieldVisualCell(side,row,col)` → 视觉格（`visualRow=(深度-1)-col`·`visualCol=row`）；均匀网格用 `s7FieldUniformPos(...)`。战斗演出 `fieldPlayerPos/fieldEnemyPos` 与推演页/备战九宫格**共用此真源**，改朝向/格数只改 `S7BattleFieldOrient` 一处。

## B0.8 · 展示立绘朝向公约（2026-07-06·Ron 拍板·装配页首例）

> 管**展示型 3/4 立绘**（单舰装配页为首例，后续同类展示页沿用）；战场格子朝向归 B0.7，两者互不覆盖。出图规格词必须写死本条。

1. **星舰 3/4 立绘：机头一律朝画面左下**，舰尾/翼展朝右上（不顶右上信息区）。
2. **驾驶员站舰右前方**（画面右侧）：视线动线顺到右下主行动键（B0.6 #14 主键恒右），并与面板"驾驶员槽"同侧呼应。
3. **驾驶员头身比：暂定 2–2.5 头身**（=第22批标准"童身大人派头"；Ron 2026-07-06 暂定，正式锁样时移交附录A 人设规格为准）。
4. 先例：第22批装配页四抽=合规样例；第20批（机头右下+人左）=历史镜像，不再沿用。
...
- **居民人气（Ron 2026-07-06 拍板·美术阶段实现）**：主界面要有 Q 萌小居民在建筑间走动/干活（对齐全局风格基准海报的基地一角）；**可见居民数量随居住舱人口增长**（少→冷清、多→热闹，档位映射第三块/美术阶段定），让"星港越来越热闹"肉眼可见。灰盒阶段不做；出图基准与量产素材从第17批R3起必须包含小居民。
- **今日补给箱**（活动区旁的小礼盒岛）：每日 1 次看广告开（S13 #2）；**零红点**（S13.1 广告零红点铁律），已开则当日隐入背景；**持广告券恢复显示+标「广告券×N」**（S13 三态·块5 已实现：灰盒=活动区下方细条礼盒岛，"空盒隐入背景"演出留美术阶段）。
- 底部：`背包`（资源/宝箱，新增挂 `Badge-Count`）+ `邮件`（未读数 `Badge-Count`）。
- 上线时：`回港报告弹窗`（L2，自动弹，见 B4.4·包A 改造）。**星港趣事**：进 hub 偶发（每日≤1）小弹泡：居民/驾驶员 Q 版头像+**speaker 显示名**+一句趣话+微量奖励，点一下收下（L2 轻浮层，不打断操作；块5 已实现=独立 `S7AnecdoteBubbleView`·货币条下方圆角泡·只有泡本体可点·回港报告在场时让位；**巡检批 2026-07-06**：正式文案池 71 条接入 + 弹泡加 speaker 名行〔驾驶员显本名·居民去「居民-」前缀〕，头像仍占位 emoji）。
- 〔DEV-TEMP·上线删〕底部小工具行（块5 现况 8 槽）：测回港 / 测邮件 / 回n029 / 刷悬赏 / 必遇袭 / +5券 / 重置广告 / 触发趣事。
...
- 技能/炮火特效（**原地发射投射物/能量，无近身砸**，对齐 D 写作铁律⑥；弹种/四件套/V 档演出走 `GDD-附录D-战斗视觉总谱.md`——原通用攻击视觉库已并入）。
- `倍速键` 1/2/3x（右下，循环切）；`跳过` 键；受击反馈=特效强度（**不做伤害飘字**·Ron 2026-07-07 拍板，总谱 §3）。
- 计时/胜负：清场=胜、超时 **120s** 判负；平时不显计时、剩 30s 才浮现倒计时+边缘微红呼吸（总谱 §6）。
- **战场背景动效层**（Ron 2026-07-07 立项）：三层视差星流+速度情绪曲线+护栏/降级开关——口径全在总谱 §4。
- **单位战斗 HUD（Ron 2026-06-25 拍板 · `Unit-BattleHUD`）**：每艘我方星舰**上方**一组「**驾驶员头像（左）+ 血条（右）**」（传统"头像+血条"组合，小而统一、别太大）。**头像=驾驶员（认人/身份）；血条=星舰的血**（驾驶员不加血，对齐附录D）；**星舰本体不再画驾驶员**（呼应附录D"星舰弱拟人·不给脸、驾驶员承载情感"）。血条可预留挂**极小状态图标**（中毒/护盾/buff）与**技能 CD 细条**（要"看懂质变"时再加，MVP 头像+血条即可）；**装核星舰血条旁挂一个小星核宝石图标**（充能变亮·发动瞬间从此弹出质变演出——总谱 §2b「HUD 图标+发动弹出」规则·2026-07-07；"挂件"路线作废）。**敌方简化**：小怪只给细血条、**头目才给头像**（省素材、分主次）。固定阵位（不移动）正好留得出头顶空间。
- **召唤单位 HUD（Ron 2026-07-06 明确·战斗规则入 UI 真源）**：工程舰无人机等**召唤单位=独立作战单位**，生成在**离召唤者最近的我方九宫格空闲格**上（就近原则·Ron 2026-07-06 补；各占独立格位，不是贴身漂浮的宠物）；HUD 只给**极小细血条、无驾驶员头像**（头像仅有人驾驶的星舰）；体型明显小于星舰。出图与工程实现同此口径。
...
**④ 视觉**：Q萌但有火力感；明亮特效（激光/炮弹/能量束）；**星核质变要"画面更爽"**（如陨星弹把普攻变陨星轰击·响度=总谱 V3）；入场滑入；命中处特效——"这一下不一样"由特效强度承载（**不做伤害飘字**，总谱 §3 反馈五件套）。星舰=载具不给脸、身份靠头顶头像。
...
### B1.3a 战斗演出规格（→ 2026-07-07 已迁入《GDD-附录D-战斗视觉总谱》· 本节只留结算侧条款）
> **迁移说明（Ron 2026-07-07 拍板⑥）**：战斗演出统一口径自此以 `GDD-附录D-战斗视觉总谱.md` 为唯一真源（V0-V4 响度阶梯 / 弹种四件套 / 反馈五件套 / 背景动效层 / 镜头语法 / 事件序列 / 性能红线）。原条款处置：**伤害字三级制=废除**（不做战斗伤害飘字·拍板⑦，"这一下不一样"改由特效强度承载=总谱 §3）；**震屏白名单 / 击杀标准演出 / 开场 10 秒陨星弹演出**=原样并入总谱（§1 / §8）；**破墙雪耻结算**属结算 UI、保留本节：
...
- ~~开场 10 秒陨星弹演出~~ → 原条款（B7 拍板①）已原样并入总谱 §8，此处不再重复（DRY）。
...
- 胜利**首通·三选一屏＝唯一奖励屏**（Ron 2026-07-04 真机拍板·块5 扩两键定稿）：战斗胜利后先盖 **三选一屏**（`Dlg-PickOne`）＝**唯一列奖励的地方**——顶部**固定奖励列表**（必得软货币）＋（精英/Boss）**必给大奖预告**（陨星弹/星辉货舱）＋ 三张**选卡**；**必须选 1**（吞触摸·防杀进程丢奖）。**选完（块5·全部关卡）**：三张卡**锁定**（选中高亮✓/其余灰），**同屏下方并排浮现** `Btn-Ad`「📺 **固定奖励**×2」（S13 #3）＋「📺 **再选一个**」（S13 #4）＋「继续」——两键各自每日 1 次/广告券三态/新手期隐（S13.2），**两键都不可显则选完直接走**（不留空"继续"步）。**#3 只翻固定奖励**（结算软货币＋固定补充驾驶记录/星贝·**就地翻倍＋飘字"+X→+2X"＋键变"已翻倍✓"**）；**唯一核（陨星弹）/ 三选一选中项 / Boss 星辉货舱均不翻**。**#4**＝看完广告后**剩下两张恢复可点、玩家再选一张**（第二张也锁定✓；已看广告未再选时「继续」拦下防白看）。**Boss 首通**：点「继续」后、结算卡出现前，弹**大奖特写弹窗**（大字奖名＋图标占位＋效果说明〔如陨星弹"普攻质变为原子炮"·取自星核真源〕＋「收下」·仪式感·正式演出随战斗演出块升级）。**结算卡不再列奖励明细**（奖励全在三选一屏·两屏不重复）——只留 `★战斗胜利★`banner ＋ `📊伤害统计` ＋ 三键。**行动宝藏/开箱等其它三选一不带这两键**（仅主线首通）；强引导期三选一不出广告键。
- **战败（块5 已实现·仅主线）**：结算卡下方附**安慰补给**小块（合金/驾驶记录小额，**白送直接入账**，每日 ≤3 次·当日首败额外附 1 张普通信标；第 4 败起零包只显一句鼓励文案〔柔和不挫败·④〕；随附 `Btn-Ad`「📺 安慰双倍」每日 1 次·S13 #9·三态；悬赏/回廊/推演战败不给——各有零惩罚机制）+ **败因归类＋克制建议＋跳转按钮**（B8① · 见 B1.5b：一句"为什么输 + 怎么破 + 一键去修"，**不推广告**，守五原则第 5 条；随战斗演出块落地·不在块5）。

**③ 布局骨架**：结算卡居中（标题上、战报中、伤害统计键中下、三键底排）；**三选一屏（唯一奖励屏）/ 大奖特写 / 伤害统计**浮层盖其上。

**④ 视觉**：胜=绿 + 庆祝飘字/光效（爽感）；败=**柔和不挫败**的提示（让人愿意再来一次，§产品原则六.4）；三选一卡横排吸睛。

**⑤ 交互与跳转**：结算弹窗**吞触摸**（必须点按钮、不点空白关）；`下一关`→已推进的下一关备战；`再次挑战`→重打当前；`选择关卡`→选关浮层；`📊`→伤害统计浮层；**奖励流程（首通·块5 全部关卡）：三选一屏必选 1 → 选完同屏并排浮现「📺固定奖励×2」(#3·就地翻倍)＋「📺再选一个」(#4·剩两张再选一)＋「继续」（两键按 S13 三态显隐·全隐则直接走）→ 继续后（Boss）弹大奖特写「收下」→ 露出结算卡**（唯一核/选中项/星辉货舱不翻）。🔊win/🔊lose/🔊reward/🔊pick。
