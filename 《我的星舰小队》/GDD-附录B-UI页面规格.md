...
- **作战板块（2026-07-02 包A 扩）**：`出战` 大主按钮（`Btn-Primary`·主线）+ 一行三枚次级作战入口：`深空回廊`（显当前层数）/ `每日委托`（显今日+积压剩余次数 `Badge-Count`，非闪烁红点）/ `每日推演`（显今日已解/未解）。解锁前 `State-Locked`（回廊=首Boss后、委托/推演=关5后）。
...
- 胜利**首通**：结算卡上 `Btn-Ad`「必得奖励×2」（**全部关卡**首通可用，S13 #3）；再盖一层 `Dlg-PickOne` **三选一发奖**（按档稀缺池；**必须选完才回结算**；**仅精英/Boss** 追加 `Btn-Ad`「再选一个」从剩余两项再拿一，每日 3 次，S13 #4——普通关三选一不带广告键，守"不排队"）。
- **战败**：结算卡附**安慰补给**小块（合金/驾驶记录小额，**白送直接入账**，每日 ≤3 次；随附 `Btn-Ad`「安慰双倍」每日 2 次，S13 #9）+ 一行养成建议（"试试升级/换搭配/去深空回廊练练"——**不推广告**，守五原则第 5 条）。
...
> 尚无灰盒（第2.5块建），故本组是"目标规格"而非"照实记录"；工程按此建灰盒，出图排第 4 批。

## B5.1 · 每日委托（护航 / 演习 · L1）
- **① 用途**：每天的无脑速刷档——一键开打、秒赢秒领（S10.8）。
- **② 元素**：顶部说明行（"星港委托·每日刷新"+ 当前星域档位）+ **两张委托大卡**：`护航`（产合金+少量星贝）/ `演习`（产驾驶记录），每卡=场景缩略图 + 今日剩余/积压次数（`Badge-Count`）+ 产出预览 + `一键开打`（`Btn-Primary`，**跳过备战沿用上次阵容**）；已打过当前星域档→卡上并排出现 `秒结算` 键（新看旧秒，S10.8）；全档玩熟→顶部 `一键速刷今日份`。积压提示行："攒着的任务：X 次（星港帮你留着，上限 Y 次）"——**礼物语气，不是催办**。
- **③ 布局**：说明行 → 护航卡 / 演习卡 上下两张大卡 → 底部返回。
- **④ 视觉**：**与主线强区分**（S10.8 三层区分）：护航卡=星门航道+运输船+劫掠涂装敌；演习卡=蓝色全息网格靶场+半透明靶机；整体"接单办事"的轻快感，不是"征战"的郑重感。
- **⑤ 交互**：`一键开打`→直接进战斗（30-60s，敌人压玩家战力一截必赢；护航场上运输船满血通关→加发"完美护航"小奖飘字）→结算回本页；`秒结算`→直接飘字入账；次数用尽→卡片变灰显"明天再来"（**无倒计时数字压迫**）。🔊dispatch/🔊reward。
- **⑥ 图标**：护航（运输船）、演习（靶标）、积压包裹、秒结算⚡。

## B5.2 · 深空回廊（无尽塔 · L1）
- **① 用途**：无限层长线挑战——变强验证场 + 毕业后主循环（S10.7）。
- **② 元素**：**竖向层数塔**（滚动列表，向上无限）：已过层（暗）/ 当前层（高亮大卡：敌情预览+推荐战力对比条）/ 上方几层剪影；**戏法层**（每10层）挂规则图标+一句规则说明；**回响Boss**（每25层）挂 Boss 剪影；**里程碑**（每5层）挂宝箱图标（可领时亮起，领取处 `Btn-Ad`「里程碑×2」，S13 #10）；顶部：当前层数大字 + 历史最高；底部：`挑战下一层`（`Btn-Primary`）+ 返回。
- **③ 布局**：顶部层数牌 → 塔身滚动区（主体）→ 底部挑战大钮。
- **④ 视觉**：越往上星空越深邃、层卡颜色渐变（星域主题色轮换）；戏法层用"扭曲星云"边框一眼可辨；里程碑宝箱金光；整体"探索未知深空"的神秘感（与主线"收复星域"的明亮感对比）。
- **⑤ 交互**：`挑战下一层`→走标准备战（回廊**不跳备战**——每层都值得摆阵，与委托相反）→战斗→胜=层数+1、遇里程碑弹领取；败=停留原层、结算给养成建议（**不推广告**）；戏法层卡点说明规则+提示"换搭配可解、攒战力可碾"。🔊tower_up/🔊milestone。
- **⑥ 图标**：层数、戏法层（扭曲星云）、回响Boss、里程碑宝箱、历史最高旗。

## B5.3 · 每日推演（战术谜题 · L1）
- **① 用途**：每天一道全服同题的搭配解谜——会玩的人的甜点（S10.9），**纯可选、无广告**。
- **② 元素**：题号+日期（"第 X 题·全星港指挥官同题"）+ **敌阵沙盘**（5×7 预览，威胁单位高亮+一句威胁提示）+ **候选区**（本题候选 6-10 舰/员卡，归一等级，玩家从中选 5）+ 我方 3×3 摆位格 + `开始推演`（`Btn-Primary`）+ 今日已解标记；解开→奖励卡（星贝+通用碎片小额）+"明日再会"。
- **③ 布局**：题头 → 敌阵沙盘（上）→ 候选横排（中）→ 我方摆位格（下）→ 开始键。
- **④ 视觉**：**棋局感/沙盘感**（全息投影桌面、蓝图线条），和真实战斗的实景区分——强调"这是推演不是实战"；解开时"妙手"庆祝特效。
- **⑤ 交互**：拖候选→摆位格；`开始推演`→确定性战斗回放；失败→"再想想"（**不限次数、零惩罚、无提示广告**）；成功→领奖+标记今日已解；未解完就走→明天换题不留欠账。🔊puzzle_start/🔊puzzle_solve。
- **⑥ 图标**：题号、沙盘、候选卡、已解勾。
