...
- 建筑入口岛（2 列网格 ×8）：船坞·养成 / 训练舱·驾驶员养成 / 打捞港·打捞 / 居住舱·人口 / 星港补给站·抽卡 / 商人小站·买卖 / 研究塔·升级 / 星核展厅·收藏。每枚=建筑图标 + 名 + 一行状态（`Lv.X`、可升挂 `↑红点`，§建筑升级入口决策）。
...
**③ 布局骨架**：三段式纵向。顶部货币条 → 活动两枚 → 8 建筑 2×4 网格（主体）→ 底部 背包/邮件（左）+ 出战大钮（右下拇指热区）。**基地全景立绘做背景**，建筑随等级演进（外观分级属附录A A4）。
...
**⑥ 图标**：货币×3、建筑×8、背包/邮件/活动×4、`↑可升`/红点/未读徽标。
...
- **⑤ 交互**：`管理`→单位管理面板（B2.3，内含升阶）；`升级船坞`→建筑升级弹框（B2.6）。**三笔花费分开算**：升船坞（本页 `升级船坞` 键）花**星矿**；升星舰等级（B2.3 内 `升级`）花**合金**；升阶（B2.3 内 `升阶`）花**专属碎片**——三条互不占用。船坞升级**封顶=船坞等级×5**（到顶提示升船坞）。🔊upgrade。
