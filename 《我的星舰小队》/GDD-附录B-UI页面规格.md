...
- 胜利**首通·两段式**（Ron 2026-07-04 拍板·块2真机落地）：**第一段**先盖 `Dlg-PickOne` **三选一发奖**（按档稀缺池；**必须选完才回结算**·防杀进程丢奖；**三选一本身不带广告键**）；**选完回到结算卡=第二段**，**仅精英/Boss** 浮现 `Btn-Ad`「看广告·必得软货币×2」（S13 #3·**单独一行居中、在下排三键之上**）。**#3 只翻"首通必得软货币"**（结算软货币＋固定补充的驾驶记录/星贝）；**唯一核（陨星弹）/ 三选一选中项 / Boss 星辉货舱均不翻**。**普通关**：三选一（必选）→ 结算卡**不带广告键**（守"不排队"）；精英/Boss 与普通关的差别**仅在第二段有无 #3 广告键**。〔S13 #4「三选一再选一」当前两段式**暂不含·留后**。〕
...
**⑤ 交互与跳转**：结算弹窗**吞触摸**（必须点按钮、不点空白关）；`下一关`→已推进的下一关备战；`再次挑战`→重打当前；`选择关卡`→选关浮层；`📊`→伤害统计浮层；**两段式：先 三选一必选 1（`Dlg-PickOne`）→ 回结算卡后（仅精英/Boss）「看广告·必得软货币×2」(#3)**（唯一核/选中项/星辉货舱不翻）。🔊win/🔊lose/🔊reward/🔊pick。
