...
14. **弹窗按钮组统一语法**（2026-07-06 Ron 点名立规）：同一弹窗内按钮**等宽等高、同一水平轴、对称分布**——1 键居中、2 键左右对称等分、3 键等分一行；语义位置固定：**主行动/确认键恒在右（单键则居中）、取消/返回键恒在左**；主键暖色实心、次键冷色/描边，全游戏不反转（反例：两键弹窗一个居中一个靠右的不对称混排）。**实现唯一真源（巡检批 2026-07-06 立）**：`S7DemoController.dialogBtnRowXs`（1/2/3 键孔位）+ `addDialogBtnPair`（静态两键）+ `layoutResultKeys`（结算窗随语境重排）——今后弹窗按钮组一律走它们排布，禁止手摆坐标。
