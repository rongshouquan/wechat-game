...
> **B5.1 星港悬赏板 + B5.2 深空回廊已建灰盒**（第2.5块·块2/块3）转为"照实记录"；B5.3 每日推演仍是"目标规格"（第2.5块后续块建），工程按此建灰盒，出图排第 4 批。
...
- **② 元素**：**竖向层数塔**（向上无限）：当前挑战层（高亮大卡：层号 + 类型〔普通/戏法·规则+解法/回响Boss〕+ 敌情概要）/ 上方几层剪影（戏法⚔ / 回响Boss👑 / 里程碑🎁 标记，让玩家看到前方）；**里程碑宝箱区**（每5层攒一个·可积攒·手动开：每个一行 奖励文案 +「开箱」+「开箱×2 📺」`Btn-Ad`·S13 #10）；顶部层数牌（下一层 + 历史最高）；底部：`挑战下一层`（`Btn-Primary`）+ 返回星港 + DEV 跳层行。
