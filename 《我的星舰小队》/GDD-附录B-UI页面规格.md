...
- 技能/炮火特效（**原地发射投射物/能量，无近身砸**，对齐 D 写作铁律⑥）。
- `倍速键` 1/2/3x（右下，循环切）；`跳过` 键；受击数字飘字。
- 计时/胜负：清场=胜、超时 **120s** 判负。

**③ 布局骨架**：沿用备战屏坐标，我方下/敌方上，投射物来回。倍速/跳过在右下安全热区（避顶部安全区）。

**④ 视觉**：Q萌但有火力感；明亮特效（激光/炮弹/能量束）；**星核质变要"画面更爽"**（如过载核心把普攻变原子炮）；入场滑入；命中处特效 + 伤害飘字（情绪价值，不改数值）。
