...
- **🔄 块6 基地养成 + 货币对齐（进行中；货币映射已整体拍板见 §待拍板①/路线图§4；迁移=直接重置，已 push 远程备份）**：
  ...
  - 🔄 **块6b 建筑运行时（进行中；拆 6b-1 集对齐 / 6b-2 运行时骨架+存档 / 6b-3 数值落地 / 6b-4 离线产出+居民工人）**：
    - ✅ **块6b-1 建筑集对齐 v1.0 §7 八栋（已 push, be4eec2）**：建筑配置表原是 Codex「7 默认 + 2 奇迹预留 = 9」且含已取消的指挥塔/星港/观星台、缺驾驶员训练舱。本块对齐成 v1.0 钦定 **8 栋**(船坞/驾驶员训练舱/居住舱/星港补给站/打捞港/商人小站/研究塔/星核展厅=**7 默认 + 1 条件预留**)：删 `command_tower`(→主界面UI按钮,非建筑)/`observatory`(奇迹概念取消)；`starport`→`supply_station`(星港补给站,解锁移 D1)；`core_gallery` 组 miracle→**showcase**、去奇迹材料、转「条件预留普通建筑」(保 `bld_rsv_*`+post-D7,仍排除出免广告核心结构)；**新增 `pilot_training_bay`**(驾驶员训练舱,D1-2,槽 pilot_upgrade/starup/detail)。组枚举删 core_entry/base_expansion/miracle、加 pilot_growth/supply_comfort/showcase；6 张配置表(含 unlock_checkpoint 桥接)+TS/.mjs 双校验器+s7_config 测试全同步。**风控不变量保持**(建筑不卡主线/no-ad、requiredLevelCap≤1、corePathRequired 仅 dock)。`npm test` **67套件/741测试全绿** + 双校验器绿，零回归。**注**：此层是「标签层=合规风控锚点」，**仍不写数字**；真实成本曲线/产率/各级效果值留 **6b-3**(走新纯 TS 运行时模块,不污染风控层)。
    - ✅ **块6b-2 建筑运行时骨架 + 存档 v3→v4（已 push, ddc2848）**：新增 `core/s7/S7BuildingState.ts`(纯TS、与配置解耦：buildingId→等级1-10 map + createDefault/normalize/解锁(蓝图激活1级幂等)/取等级/可升判定/升级) + `core/s7/S7BuildingUpgradeService.ts`(`upgradeBuilding(state,resources,id,cost)`：校验 已解锁→未满级→成本合法→星矿够 → 扣星矿+升1级；5 类结果失败不改动；**成本由调用方传入=6b-3 真实星矿曲线**)。S7 存档 `S7PlayerState` 加 `buildings`、版本 **v3→v4**(加性迁移、旧档补默认空建筑)。**顺手修了 6d-1 遗留 bug**：`persistS7Save` 落盘漏写 `pluginInventory`(会丢插件库存)→补 pluginInventory+buildings、加往返测试锁住。新增 `s7_building_state` 11 例(含 `constructor` 脏键真正验到防护) + 2 手写 `.ts.meta`。`npm test` **68套件/754测试全绿** + 校验器绿，零回归。
    - ✅ **块6b-3 建筑数值落地（已 push, 19613b1）**：新增 `core/s7/S7BuildingCost.ts`(成本=`round(120×L^1.3×重要度系数)`纯星矿；核心船坞/训练舱×1.3、重要居住舱/研究塔/补给站×1.1、普通×1.0；满级/未解锁/非整数→null) + `core/s7/S7BuildingEffects.ts`(纯函数 等级→数值：船坞/训练舱 等级上限=楼级×5对齐B1主力线、居住舱离线存储36→48h+产率2%/级、打捞队1/2/3、研究塔+1%/级、星核展厅"收藏图鉴"每种0.3→0.6%×种数封顶10%、补给站A级/3★+0.5%/级占位、商人槽位2→6/刷新1/2/3)。清 `building_level_cost_param` 升级段占位材料标签(纯星矿)。新增 `s7_building_numbers` 11 例(成本独立重算核验)+2 手写 meta。数值文档→v0.2 定稿。`npm test` **69套件/765测试全绿**+校验器绿，零回归。**各级效果的"被消费"(离线/战斗/星舰升级/抽卡)随对应系统建好时接入；数值 v0.1 待原型校准。**
    - ✅ **块6b-4a 离线产出纯函数 + 星域系数（已 push, 1f34275）**：新增 `core/s7/S7OfflineProduction.ts`(纯TS·确定可复算 §12.5)：`computeOfflineGains(经过秒数,{habitatLevel,clearedStarfieldTier,extraRateBonusPct?,extraStorageHours?})`→软货币(星矿/合金/驾驶记录)进账；速率=基础×**星域系数(随已通关最高星域,主线进度永久乘区)**×(1+居住舱产率+额外)；上限=居住舱存储(+额外)h、溢出标记。`starfieldCoefficient`(0-4→1.0/1.6/2.6/4.0/6.0)。`S7MainlineModel.clearedStarfieldTier`(各星域最后节点 sf01→n018/sf02→n037/sf03→n056/sf04→n075 是否通关→最高档)。
    - ✅ **块6b-4b 基地人口(居民/工人) + 存档 v4→v5（已 push, 250f0ce）**：新增 `core/s7/S7Population.ts`：居民 离线产率+1%/人(封顶+40%)·离线存储+0.25h/人(封顶+12h,守§16红线 48+12=60h≈2.5天)；工人 建筑升级-0.5%/人(封顶-30%)+`applyWorkerDiscount`。居民经离线 extra* 注入、工人折扣叠在 S7BuildingCost 之上(组合式,不破坏解耦)。存档加 `population`、版本 v4→v5(加性迁移补默认0)。
    - ✅ **块6b 建筑 整体完成**(集对齐/运行时骨架+存档/数值/离线产出/人口)。居民工人"数量"来自主线救回+7天扩张(内容侧后续)；各级效果/离线/人口的"被消费"(上线弹窗领离线、建筑升级UI、战斗结算用研究塔展厅加成、星舰/驾驶员升级用等级上限)随对应系统建好接入；数值v0.1待原型校准。
    - **本轮拍板留痕(2026-06-19)**：①数值真源铁律=**B1为准/v1.0设计真源/旧Codex配置(如growth_band等级驱动战力)自由改或无视**(见 memory + 决策档)；②满编5套**压到D1-2**(§11)；③建筑升级**纯星矿**、成本按重要度3档；④星核展厅改**收藏图鉴**(保留等级、等级放大每种加成,反与研究塔冗余)；⑤星港补给站升级**提A级/3★率**(温和,并入§10.1)；⑥星核展厅解锁挂**首核(约D3)**非D7笔误。
...
- **📌 当前总览（2026-06-19 收口）**：块0-5 全 + 货币映射拍板 + 块6a + 块6d + **块6b 建筑整体完成(6b-1集对齐/6b-2骨架+存档/6b-3数值/6b-4a离线产出+星域系数/6b-4b居民工人)**。**`npm test` 71 套件 / 780 测试全绿** + 双配置校验器绿，一路零回归，**全部已 push**（本地=远程 `origin/main`，最新 `250f0ce`）。四层全接进战斗；插件品质/合成/回收/背包立起；**建筑：8栋对齐v1.0 + 状态/解锁/升级 + 成本曲线/各级效果 + 离线产出(随主线星域抬升)/居民/工人 全落地**(纯星矿；各级效果/离线/人口待对应系统接入消费；数值v0.1待原型校准)。存档版本至 **v5**。修复 persistS7Save 漏存 pluginInventory 旧 bug。
- **🧭 角色与协作（2026-06-18 起·重要，见全局 CLAUDE.md 顶部）**：Claude 负责**项目顶层设计、数值体系搭建、养成、全套工程代码、合规风控/平台规范、复杂问题排错重构优化、长周期迭代规划**；**Ron 是总决策人**——Claude 出的方案先用大白话精简讲清、经 Ron 确认后执行；觉得哪块适合 Codex 就主动提。**设计分工**：玩法策划、广告策略 **以 Ron 为主**；关卡设计、UI **以 Claude 为主**；顶层/数值/养成/规划 Claude 主导——全部 Ron 总决策拍板。**关键转变：数值/顶层方案由 Claude 出草案、Ron 拍板，不再"等 Ron 给数值"**（之前会话里"数值是 Ron 的活、我不擅编"的说法已作废）。
- **🔜 下一步候选（新对话从这里挑；不再有"卡死等数值"项）**：
  1. ✅ **块6b 建筑 整体完成**（6b-1集对齐 / 6b-2骨架+存档 / 6b-3数值 / 6b-4a离线产出+星域系数 / 6b-4b居民工人）。
  2. **块6 余项**：货币新增键(starGem/通用驾驶员碎片/信标拆3档)、独立结构(专属碎片/居民工人/宝箱)——由 Claude 连 anchor 毕业预算数值一并出草案、Ron 拍板。
  3. **块7 活动**(3天·7天，去每日任务)——靠前面产出，内容偏后。
  - **建议起点**：块6 余项（货币新增键 starGem/通用驾驶员碎片/信标拆3档 + 独立结构 专属碎片/宝箱）连 anchor 毕业预算数值出草案给 Ron 拍；或块7 活动；或 Ron 指定优先级。**注：居民/工人状态+效果已就绪，缺的是"数量从哪来"=主线救回/7天扩张的内容侧产出。**
...
## Git
每个里程碑已提交（`git log` 见 `《我的星舰小队》:` 系列）。**块0 至 块6b（建筑整体）全部已 push**（本地 main == `origin/main`，最新 `250f0ce`「C1块6b-4b 基地人口 + 块6b 完成」）。新增改动继续按里程碑提交、干净节点 push 备份。
> 注：仓库根在 `F:\Claude Code`（remote `rongshouquan/wechat-game`），同目录另有无关内嵌仓库 `untangle`（非本项目，勿动）。
