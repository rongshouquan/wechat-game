...
- **🔄 块6 基地养成 + 货币对齐（进行中；货币映射已整体拍板见 §待拍板①/路线图§4；迁移=直接重置，已 push 远程备份）**：
  ...
  - 🔄 **块6b 建筑运行时（进行中；拆 6b-1 集对齐 / 6b-2 运行时骨架+存档 / 6b-3 数值落地 / 6b-4 离线产出+居民工人）**：
    ...
    - 🔜 **6b-3 起**：数值落地(纯星矿成本曲线 1→10 级 + 各级效果数值，Claude 出草案给 Ron 拍；顺带清配置占位"建筑材料"标签) → 6b-4 离线产出(纯函数:经过秒数,状态→资源,上限由居住舱)+居民/工人，替换旧采矿站(挂 PlayerState 产旧能量)路径。
    - **拍板留痕**：建筑升级**纯星矿**(2026-06-18，去 v1.0 §7 原"少量指定材料"，已落设计 §7+§17)；星核展厅解锁挂**首核(首个 Boss 首杀,约 D3)**、拿到首核当下即引导(订正 §7 原"D7"笔误)。
...
- **📌 当前总览（2026-06-18 收口）**：块0-5 全 + 货币映射拍板 + 块6a(砍星核强化+删废弃币) + 块6d(插件合成/回收/背包) + **块6b-1(建筑集对齐 v1.0 八栋) + 块6b-2(建筑运行时骨架+存档 v3→v4)** 全部完成。**`npm test` 68 套件 / 754 测试全绿** + 双配置校验器绿，一路零回归，**全部已 push**（本地=远程 `origin/main`，最新 `ddc2848`）。四层(星舰·驾驶员·插件·星核)全接进战斗；插件 品质/合成/回收/背包 立起；建筑集对齐 v1.0(8 栋) + 建筑状态/解锁/升级骨架与存档已立(数字待 6b-3)。**另：修复 persistS7Save 漏存 pluginInventory 的旧 bug。**
...
- **🔜 下一步候选（新对话从这里挑；不再有"卡死等数值"项）**：
  1. **块6b 续做（6b-1 集对齐 ✅ / 6b-2 运行时骨架+存档 ✅）**：→ **6b-3 数值落地**(纯星矿成本曲线 1→10 级 + 各级效果数值，Claude 出草案给 Ron 拍；顺带清配置占位"建筑材料"标签) → **6b-4 离线产出**(纯函数:经过秒数,状态→资源,上限由居住舱)+居民/工人,替换旧采矿站(挂 PlayerState 产旧能量)。**建议从 6b-3 起（需 Ron 拍数值草案）。**
  2. **块6 余项**：货币新增键(starGem/通用驾驶员碎片/信标拆3档)、独立结构(专属碎片/居民工人/宝箱)——由 Claude 连 anchor 毕业预算数值一并出草案、Ron 拍板。
  3. **块7 活动**(3天·7天，去每日任务)——靠前面产出，内容偏后。
  - **建议起点**：6b-3 数值落地（Claude 先出纯星矿成本曲线 + 各级效果草案给 Ron 拍）；或 Ron 指定优先级。
...
## Git
每个里程碑已提交（`git log` 见 `《我的星舰小队》:` 系列）。**块0 至 块6b-2 全部已 push**（本地 main == `origin/main`，最新 `ddc2848`「C1块6b-2 建筑运行时骨架 + 存档 v3→v4」）。新增改动继续按里程碑提交、干净节点 push 备份。
