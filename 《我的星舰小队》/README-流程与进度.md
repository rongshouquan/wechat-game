...
- **🔄 块6 基地养成 + 货币对齐（进行中；货币映射已整体拍板见 §待拍板①/路线图§4；迁移=直接重置，已 push 远程备份）**：
  ...
  - 🔄 **块6b 建筑运行时（进行中；拆 6b-1 集对齐 / 6b-2 运行时骨架+存档 / 6b-3 数值落地 / 6b-4 离线产出+居民工人）**：
    ...
    - 🔜 **6b-2 起**：建筑运行时骨架(S7BuildingState 等级/建造/升级/解锁判定+扣资源,纯TS)+存档 v3→v4 加性迁移 → 6b-3 数值落地(成本曲线+各级效果,Claude 出草案) → 6b-4 离线产出(纯函数:经过秒数,状态→资源,上限由居住舱)+居民/工人,替换旧采矿站(挂 PlayerState 产旧能量)路径。
...
- **📌 当前总览（2026-06-18 收口）**：块0-5 全 + 货币映射拍板 + 块6a(砍星核强化+删废弃币) + 块6d(插件合成/回收/背包) + **块6b-1(建筑集对齐 v1.0 八栋)** 全部完成。**`npm test` 67 套件 / 741 测试全绿** + 双配置校验器绿，一路零回归，**全部已 push**（本地=远程 `origin/main`，最新 `be4eec2`）。四层(星舰·驾驶员·插件·星核)全接进战斗；插件 品质/合成/回收/背包 的存档与逻辑已立起来；建筑集已对齐 v1.0(8 栋,数字仍占位待 6b-3)。
...
- **🔜 下一步候选（新对话从这里挑；不再有"卡死等数值"项）**：
  1. **块6b 续做（建筑集已对齐=6b-1 ✅）**：→ **6b-2 建筑运行时骨架**(S7BuildingState 等级/建造/升级/解锁判定+扣资源,纯TS)+存档 v3→v4 加性迁移 → **6b-3 数值落地**(成本曲线+各级效果,Claude 出草案给 Ron 拍) → **6b-4 离线产出**(纯函数:经过秒数,状态→资源,上限由居住舱)+居民/工人,替换旧采矿站(挂 PlayerState 产旧能量)。**建议从 6b-2 起。**
  2. **块6 余项**：货币新增键(starGem/通用驾驶员碎片/信标拆3档)、独立结构(专属碎片/居民工人/宝箱)——由 Claude 连 anchor 毕业预算数值一并出草案、Ron 拍板。
  3. **块7 活动**(3天·7天，去每日任务)——靠前面产出，内容偏后。
  - **建议起点**：6b-2 建筑运行时骨架（6b-1 集对齐已完成）；或 Ron 指定优先级。
...
## Git
每个里程碑已提交（`git log` 见 `《我的星舰小队》:` 系列）。**块0 至 块6b-1 全部已 push**（本地 main == `origin/main`，最新 `be4eec2`「C1块6b-1 建筑集对齐 v1.0 八栋」）。新增改动继续按里程碑提交、干净节点 push 备份。
