...
4. 🔄 **逐块改造中**（顺序已定，从块0起；货币映射§4 待拍板，块6 前需定）：
   ...
   - 🔄 **块2 能量条→三类触发**（拍板：CD 走显式字段=B 方案 / 开局即放 / 星核钩子平移留块3；详见 `块2-能量改三类触发-设计草案-v0.1.md`）：
     ...
     - ▶ **块2a-2（高风险，开工前再确认）**：引擎删能量(回能/满能放招/涨能/energy字段/energy_change日志/finalState.energy) → 加 `stepTriggers`(开局即放 + CD 读 ultimateCdSec + 血量阈值) + 星核钩子平移；重定基约 31 个战斗测试（守"不为绿而绿"纪律）。
     - 块2b：on_kill/on_hit/友军阵亡 条件型 + 装配条件型积木。
