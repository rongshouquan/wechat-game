...
- **块4a 已完成（已提交，未 push）**：**插件运行时接线 + 品质制**——新增 `core/s7/S7PluginEffects.ts`(插件实例 词条+槽位+品质 → 效果积木，按品质缩放数值、传奇额外加一条小效果) + lineup 新增 `plugins?` + 装配器 `buildPlayerUnits` 解析插件积木(校验：≤3/未知/同名/同槽/品质)并与星核积木合并喂引擎。
- **块4b 已完成（已提交，未 push）**：**引擎消费全部 8 条定向词条**。4b-1：`RtUnit` 加 `affixes`(spawnUnit 从 deriveUnit 拷，无装配=ZERO_AFFIXES) + `dealDamage` 消费对Boss/对小怪加伤(按 `isBoss` 路由) + `heal` 消费治疗强度。4b-2：`dealDamage` 加破盾值(叠 shieldMult)、暴击(critRate>0 才掷 RNG→短路保零回归、命中×(1+critDmg)、暴击时日志带 `crit:true`)；`applyState` 加控制抗性(只缩 `CONTROL_TAGS=short_circuit/stun` 时长)；`fireTrigger` 加技能急速(触发 CD `/(1+skillHaste)`)。`npm test` **63 套件 / 720 测试全绿** + s7 校验器绿，零回归。
- **块4c-1 已完成（已提交，未 push）**：**插件槽名 `energy`→`skill`（技能/CD槽）对齐 v1.0 §5.3**——改 `S7PluginSlot` 类型、TS+.mjs 双校验器槽枚举、`S7PluginEffects` 槽键、plugin_config 4 行、相关测试。`npm test` **63 套件 / 720 测试全绿** + s7 校验器绿，零回归。
- **下一步 → 块4c-2：清理旧「插件强化 1-15 级」制**（与 v1.0「插件不分等级」冲突）——删 `enhance_cost_param`/`growth_band_param` 的 **plugin 行**（保留 core 强化，那是另一回事）、TS+.mjs 双校验器去 plugin 强化/成长校验、输入契约去 `pluginEnhanceById`，并按"插件本就无强化"如实重定基 `s7_growth_band`/`s7_config`/`s7_battle_input_snapshot`/`s7_battle_input_run_request_adapter` 4 个测试。**块4d 合成/回收/背包已决定挪入块6**。其后：块5 驾驶员运行时 / 块6 基地养成+货币(含 4d) / 块7 3天·7天活动(去每日任务)。
...
4. 🔄 **逐块改造中**（顺序已定，从块0起；货币映射§4 待拍板，块6 前需定）：
   ...
   - 🔄 **块4 插件品质制（拆 4a/4b/4c 战斗侧本批 + 4d 挪块6）**：
     ...
     - 🔄 **块4c 配置/校验对齐 v1.0（拆 4c-1 槽名 / 4c-2 清强化）**：
       - ✅ **块4c-1**（2026-06-18）：插件槽名 `energy`→`skill`(技能/CD槽)。改 `S7PluginSlot` 类型、`ConfigValidatorS7`+`validate-s7-configs.mjs` 槽枚举、`S7PluginEffects` 槽键+注释、plugin_config 4 行(plg07/11/13/18)、`s7_plugin_quality` 测试引用。63套件/720测试全绿+校验器绿，零回归。
       - 块4c-2：清旧「插件强化 1-15 级」制(与「不分等级」冲突)。删 `enhance_cost_param`/`growth_band_param` 的 plugin 行(留 core 强化)、TS+.mjs 去 plugin 强化/成长校验(`pluginMaxEnh===15`、`byTarget.plugin`、`checkGrowthBandCoverage(...1,15)`、`S7_GROWTH_TARGET_TYPES`/`EXPECTED_SECONDARY` 的 plugin)、输入契约去 `pluginEnhanceById`+`plugin_enhance_unknown_ref`，按真实行为重定基 4 个测试。
