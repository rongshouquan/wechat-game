...
- **下一步 → 块4c：槽名 `energy`→技能(CD)槽 对齐 v1.0 §5.3 + 顺手清理旧「插件强化1-15级」制**(与 v1.0「不分等级」冲突；动 config + TS/.mjs 双校验器)。**块4d 合成/回收/背包已决定挪入块6**。其后：块5 驾驶员运行时 / 块6 基地养成+货币(含 4d) / 块7 3天·7天活动(去每日任务)。
