...
- **块4b-1 已完成（已提交，未 push）**：**引擎消费定向词条（确定性纯乘子组）**——`RtUnit` 加 `affixes`(spawnUnit 从 deriveUnit 拷，无装配=ZERO_AFFIXES)；`dealDamage` 消费 **对Boss加伤/对小怪加伤**(按 `isBoss` 路由)、`heal` 消费 **治疗强度**。`npm test` **63 套件 / 715 测试全绿** + s7 校验器绿，零回归。
- **下一步 → 块4b-2：消费余下 3 条要新约定的词条**——破盾值(shieldBreak,叠加 shieldMult)、暴击(critRate/critDmg,引入随机数+暴击倍率约定)、控制抗性(controlResist,缩 `CONTROL_TAGS` 控制时长)、技能急速(skillHaste,缩触发 CD)。**(4b 拆 4b-1/4b-2 因 8 词条挂 5 个结算点、3 条需新约定，拆小利于深度自检)** → **块4c：槽名 `energy`→技能(CD)槽 对齐 + 顺手清理旧「插件强化1-15级」制**(与 v1.0「不分等级」冲突)。**块4d 合成/回收/背包已决定挪入块6**。其后：块5 驾驶员运行时 / 块6 基地养成+货币(含 4d) / 块7 3天·7天活动(去每日任务)。
...
4. 🔄 **逐块改造中**（顺序已定，从块0起；货币映射§4 待拍板，块6 前需定）：
   ...
   - 🔄 **块4 插件品质制（拆 4a/4b/4c 战斗侧本批 + 4d 挪块6）**：
     ...
     - 🔄 **块4b 引擎消费定向词条（拆 4b-1/4b-2，因 8 词条挂 5 个结算点、其中 3 条要新约定）**：
       ...
       - 块4b-2：余下 3 条要新约定的——破盾值(叠加 shieldMult)、暴击 critRate/critDmg(引入 RNG + 暴击倍率约定 + 暴击日志)、控制抗性 controlResist(缩 `CONTROL_TAGS=['short_circuit','stun']` 时长)、技能急速 skillHaste(缩触发 CD)。
