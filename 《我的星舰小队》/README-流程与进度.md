...
- **已完成并 push**（GitHub `rongshouquan/wechat-game` main）：底座复制+跑通 → **块0 战场对齐**(敌方 5×7 + 超时 120s) → **块1 效果装配层**(四类积木→`deriveUnit`) → **块2 能量条→三类触发**(被动/条件/CD，取消能量) → **块3 星核运行时 + 新手核「过载核心」**(普攻变原子炮)。`npm test` **61 套件 / 698 测试全绿** + 双 `.mjs` 校验器绿，一路零回归。
- **下一步 → 块4：插件品质制(精良/优秀/传奇) + 合成**。其后：块5 驾驶员运行时 / 块6 基地养成 + 货币 / 块7 3天行动·7天扩张(去每日任务)。
...
- **待拍板 / 遗留**：① **货币命名映射待 Ron 拍板**（块6 前必需，路线图§4）；② core01-06 已注册但暂无战斗质变、过载核心"Boss首杀掉落"属奖励系统未做（块3 只做装备即生效）；③ n001 仍分批刷怪（与 v1.0 一次性冲突，留内容块）；④ `npm run typecheck` 须先用 Cocos 打开生成 `temp/declarations/cc*` 才过（缺 cc 类型，与逻辑无关）；⑤ 用 Cocos Creator 3.8.8 打开工程做导入/构建/真机确认仍需 Ron 手动一次。
...
4. 🔄 **逐块改造中**（顺序已定，从块0起；货币映射§4 待拍板，块6 前需定）：
   ...
   - 其后：块4 插件品质制+合成 → 块5 驾驶员运行时 → 块6 基地养成+货币 → 块7 3天7天活动(去每日任务)。
