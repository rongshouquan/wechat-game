...
- **🔄 块6 基地养成 + 货币对齐（进行中；货币映射已整体拍板见 §待拍板①/路线图§4；迁移=直接重置，已 push 远程备份）**：
  ...
  - 🔄 **块6b 建筑运行时（进行中；拆 6b-1 集对齐 / 6b-2 运行时骨架+存档 / 6b-3 数值落地 / 6b-4 离线产出+居民工人）**：
    ...
    - 🔜 **6b-4 离线产出 + 居民/工人**：纯函数(经过秒数,状态→资源,上限由居住舱 offlineStorageHours/产率)+居民(产率↑/存储↑)/工人(建筑材料即星矿成本↓)，替换旧采矿站(挂 PlayerState 产旧能量)路径。
...
- **📌 当前总览（2026-06-19 收口）**：块0-5 全 + 货币映射拍板 + 块6a + 块6d + **块6b-1/6b-2/6b-3(建筑集对齐 + 运行时骨架+存档v4 + 数值落地)** 全部完成。**`npm test` 69 套件 / 765 测试全绿** + 双配置校验器绿，一路零回归，**全部已 push**（本地=远程 `origin/main`，最新 `19613b1`）。四层全接进战斗；插件品质/合成/回收/背包立起；**建筑：8栋对齐v1.0 + 状态/解锁/升级骨架 + 存档v4 + 成本曲线/各级效果数值全落地**(纯星矿,各级效果待对应系统接入消费)。修复 persistS7Save 漏存 pluginInventory 旧 bug。
...
- **🔜 下一步候选（新对话从这里挑；不再有"卡死等数值"项）**：
  1. **块6b 续做（6b-1集对齐 ✅ / 6b-2骨架+存档 ✅ / 6b-3数值 ✅）**：→ **6b-4 离线产出 + 居民/工人**(纯函数:经过秒数,状态→资源,上限由居住舱 offlineStorageHours/产率; 居民产率↑存储↑/工人星矿成本↓; 替换旧采矿站挂PlayerState产旧能量)。**建议从 6b-4 起（建筑这块收尾）。**
  2. **块6 余项**：货币新增键(starGem/通用驾驶员碎片/信标拆3档)、独立结构(专属碎片/居民工人/宝箱)——由 Claude 连 anchor 毕业预算数值一并出草案、Ron 拍板。
  3. **块7 活动**(3天·7天，去每日任务)——靠前面产出，内容偏后。
  - **建议起点**：6b-4 离线产出+居民/工人（建筑这块最后一步）；或 Ron 指定优先级。
...
## Git
每个里程碑已提交（`git log` 见 `《我的星舰小队》:` 系列）。**块0 至 块6b-3 全部已 push**（本地 main == `origin/main`，最新 `19613b1`「C1块6b-3 建筑数值落地」）。新增改动继续按里程碑提交、干净节点 push 备份。
